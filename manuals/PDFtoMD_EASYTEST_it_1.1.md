...
6.  In accordo alla norma EN61008, il test per RCD selettivi richiede un intervallo di 60 secondi tra i test. Il modo (Corrente crescente / Rampa) non è quindi disponibile per RCD selettivi di ogni tipo.
...
La normativa IEC62955 definisce il tempo e la corrente di intervento per gli RCD - DD (Detecting Devices) alla corrente nominale di 6mA. In questa modalità sono disponibili solo le opzioni x1 e (Corrente crescente / Rampa).
...
Lo strumento consente la misura del tempo e della corrente di intervento per gli RCD di tipo CCID (forma d’onda sinusoidale) o CCID (forma d’onda continua) alle correnti nominali di 5mA o 20mA. In questa modalità sono disponibili solo le opzioni x1 e (Corrente crescente / Rampa).
...
| Modo      | TT Condizioni per esito OK                                      | TN Condizioni per esito OK                                      | IT Condizioni per esito OK                                      |
| :-------- | :-------------------------------------------------------------- | :-------------------------------------------------------------- | :-------------------------------------------------------------- |
| **L - L** |                                                                 |                                                                 |                                                                 |
| **L1 - L2** |                                                                 |                                                                 |                                                                 |
| STD       | Nessun esito                                                    | Nessun esito                                                    | Nessun esito                                                    |
| Br.Cap    | Isc L - L max < BC                                              | Isc L - L max < BC                                              | Isc L - L max < BC                                              |
|           |                                                                 | Isc L1 - L2 max < BC                                            | Isc L1 - L2 max < BC                                            |
...
| **L1 - PE** |                                                                 |                                                                 |                                                                 |
| STD       | Nessun esito                                                    |                                                                 |                                                                 |
| Br.Cap    | Isc L - PE max < BC                                             | Isc L - PE max < BC                                             | Isc L - PE max < BC                                             |
|           | Isc L1 - PE max < BC                                            | Isc L1 - PE max < BC                                            | Isc L1 - PE max < BC                                            |
| TripT     | (Ipfc L - PE min) → Tmax → Tmax < Tlim                          | (Ipfc L - PE min) → Tmax → Tmax < Tlim                          | (Ipfc L - PE min) → Tmax → Tmax < Tlim                          |
|           | (Ipfc L1 - PE min) → Tmax → Tmax < Tlim                         | (Ipfc L1 - PE min) → Tmax → Tmax < Tlim                         | (Ipfc L1 - PE min) → Tmax → Tmax < Tlim                         |
| Ut        |                                                                 | ZL - PE < ZLimt (UK)                                            | Utmeas < Utlim                                                  |
|           |                                                                 | ZL1 - PE < ZLimt (USA)                                          |                                                                 |
| Ra        | Utlim/Ra meas = Isc L - PE MIN > Idn (RCD)                      | ZLPEmis < ZLIM (UK, AUS/NZ)                                     |                                                                 |
| 2Fili     | ZLPEmeas < ZLIM (UK, AUS/NZ)                                    | ZL1PEmis < ZLIM (USA)                                           |                                                                 |
|           | ZL1PEmeas < ZLIM (USA)                                          | Ra mis x Idn < Ut lim (altre Nazioni)                           |                                                                 |
|           | Ra meas x Idn < Ut lim (altre Nazioni)                          |                                                                 |                                                                 |
| 3Fili     | ZLPEmeas < ZLIM (UK, AUS/NZ)                                    |                                                                 |                                                                 |
|           | ZL1PEmeas < ZLIM (USA)                                          |                                                                 |                                                                 |
|           | Ra meas x Idn < Ut lim (altre Nazioni)                          |                                                                 |                                                                 |
...
3.  Premere il tasto `GO/STOP` per avviare la calibrazione. Nei campi `RL`, `RN` e `RPE` è mostrata per pochi secondi la resistenza dei puntali. Questo valore sarà sottratto automaticamente dallo strumento alla fine della misura di Loop.
    Lo strumento visualizza il simbolo “`OK`” ad indicare l’esito positivo della calibrazione dei cavi di misura (`Rcal <1 Ω`) e sul display compare la videata a lato.
    Tornare alla videata principale di misura. Notare il simbolo “`OK`” che indica la corretta calibrazione dei puntali e procedere con le misure descritte nei paragrafi successivi.
...
4.  Il valore della resistenza dei puntali/spina Shuko viene mantenuto dallo strumento fino all'operazione di reset eseguita dall'utente (ad esempio per l'inserimento di cavi di diversa lunghezza).
    Per eseguire il reset del valore di calibrazione salvato, rimuovere l'accessorio `ZEROLOOP` e premere il tasto `GO/STOP`. Il simbolo “`OK`” è rimosso e la videata a lato compare sul display.
...
2.  Usare i tasti ``, `` per selezionare il parametro da modificare e i tasti ``, `` per modificare il valore del parametro:
    *   `FUN Z` → il tasto virtuale permette di impostare la modalità di misura dello strumento, che può essere: L - N, L - L o L - PE (sistemi Monofase/Trifase) oppure L1 - PE, L1 - L2 (sistemi Bifase).
...
I_SCL_PE = U_NOM / Z_L-PE_MEAS
I_SCLN = U_NOM / Z_L-N_MEAS
I_SCLL = U_NOM / Z_L-L_MEAS
```
...
1.  Premere il tasto `MENU`, spostare il cursore su `LOOP` nel menu principale tramite i tasti freccia (``, ``) e confermare con `ENTER`. Successivamente lo strumento visualizza una videata simile a quella qui riportata a lato in caso di sistema elettrico Monofase L - N - PE selezionato (vedere § 5.1.3). Per sistemi Bifase L - L - PE le tensioni indicate cambiano in VL1 - PE e VL1 - L2.
    Selezionare il paese “Europa” (vedere § 5.1.2), le opzioni “TT, TN o IT”, “25 o 50V”, “50Hz o 60Hz” e la tensione di riferimento nelle impostazioni generali dello strumento (vedere § 5.1.3).
...
1.  Premere il tasto `MENU`, spostare il cursore su `LOOP` nel menu principale tramite i tasti freccia (``, ``) e confermare con `ENTER`. Successivamente lo strumento visualizza una videata simile a quella qui riportata a lato in caso di sistema elettrico Monofase L - N - PE selezionato (vedere § 5.1.3). Per sistemi Bifase L - L - PE le tensioni indicate cambiano in VL1 - PE e VL1 - L2.
    Selezionare il paese “Europa” (vedere § 5.1.2), le opzioni “TN”, “25 o 50V”, “50Hz o 60Hz” e la tensione di riferimento nelle impostazioni generali dello strumento (vedere § 5.1.3).
    **NOTA**: per paesi diversi da "Europa" i riferimenti su MCB e Fusibile disponibili possono cambiare.
...
2.  Usare i tasti ``, `` per selezionare il parametro da modificare e i tasti ``, `` per modificare il valore del parametro:
    *   `FUN Z` → il tasto virtuale permette di impostare la modalità di misura dello strumento, che può essere: `Ra`.
...
1.  Premere il tasto `MENU`, spostare il cursore su `LOOP` nel menu principale tramite i tasti freccia (``, ``) e confermare con `ENTER`. Successivamente lo strumento visualizza una videata simile a quella qui riportata a lato in caso di sistema elettrico Monofase L - N - PE selezionato (vedere § 5.1.3). Per sistemi Bifase L - L - PE le tensioni indicate cambiano in VL1 - PE e VL1 - L2.
    Selezionare il paese “Europa” (vedere § 5.1.2), le opzioni “IT”, “25 o 50V”, “50Hz o 60Hz” e la tensione di riferimento nelle impostazioni generali dello strumento (vedere § 5.1.3).
...
2.  Usare i tasti ``, `` per selezionare il parametro da modificare e i tasti ``, `` per modificare il valore del parametro:
    *   `FUN Z` → il tasto virtuale permette di impostare la modalità di misura dello strumento, che può essere: L - PE (sistemi Monofase/Trifase) oppure L1 - PE (sistemi Bifase).
...
1.  Premere il tasto `MENU`, spostare il cursore su `LOOP` nel menu principale tramite i tasti freccia (``, ``) e confermare con `ENTER`. Successivamente lo strumento visualizza una videata simile a quella qui riportata a lato in caso di sistema elettrico Monofase L - N - PE selezionato (vedere § 5.1.3). Per sistemi Bifase L - L - PE le tensioni indicate cambiano in VL1 - PE e VL1 - L2.
    Selezionare il paese “Europa” (vedere § 5.1.2), le opzioni “TT”, “25 o 50V”, “50Hz o 60Hz” e la tensione di riferimento nelle impostazioni generali dello strumento (vedere § 5.1.3).
...
2.  Usare i tasti ``, `` per selezionare il parametro da modificare e i tasti ``, `` per modificare il valore del parametro:
    ...
    *   `IΔn` → Il tasto virtuale consente di impostare il valore nominale della corrente di intervento dell'RCD, che può essere: 6mA, 10mA, 30mA, 100mA, 300mA, 500mA, 650mA, 1000mA.
    premere il tasto `SALVA` per salvare i parametri selezionati e tornare alla videata di misura.
3.  Si possibile, scollegare tutti i carichi collegati a valle del punto misurato in quanto l'impedenza di queste utenze potrebbe falsare i risultati del test. Eseguire la calibrazione preliminare dei puntali come descritto al § 6.7.2.
4.  Inserire i connettori verde, blu e nero del cavo shuko a tre pin nei corrispondenti conduttori di ingresso B3, B4 e B1 dello strumento. In alternativa, utilizzare i singoli cavi e applicare le relative clip a coccodrillo alle estremità libere dei cavi. È anche possibile utilizzare il puntale remoto inserendo il suo connettore multipolare nel cavo di ingresso B1. Collegare la spina Shuko, i terminali a coccodrillo o il puntale remoto alla rete elettrica in accordo alle Fig. 31, Fig. 32 o Fig. 33.
...
7.  In caso di esito positivo (resistenza globale di terra `R A < (Utlim / IΔn)`), lo strumento visualizza il messaggio “OK” e la videata a lato che contiene il valore della tensione di contatto nel display secondario.
...
8.  In caso di esito negativo (resistenza globale di terra `R A > (Utlim / IΔn)`), lo strumento visualizza il messaggio “NO OK” e la videata a lato che contiene il valore della tensione di contatto nel display secondario.
...
3. Premere il tasto `GO/STOP` sullo strumento per avviare il test. La seguente videata è mostrata a display (in caso di misura L-L in modo STD).
   Sul display viene visualizzata la corrente di cortocircuito presunta standard (STD). Nella parte centrale del display sono visualizzati i valori dell'impedenza di loop L-L, oltre alle sue componenti resistive e reattive, espresse in `m`.
   ```
   L o Z   15/10   –   18:04
   TN
   Ipsc      =   15.3 kA
...
3. Premere il tasto `GO/STOP` sullo strumento o il tasto `START` sul puntale remoto. Lo strumento inizia il test.
   Il messaggio " Tocca L1 " è mostrato a display ad indicare l'attesa che lo strumento sia connesso alla fase L1 del sistema in prova. Toccare la parte attiva della fase L1.
   ```
   1 2 3   15/10   –   18:04
   TN
   -   -   -
...
4. Lo strumento emette un suono lungo fino a quando non è presente la tensione di ingresso. Al termine dell'acquisizione della fase L1, lo strumento è in attesa del segnale sulla fase L2 e mostra il simbolo di " Rilascia L1 " come mostrato nella videata a lato.
   ...
   1T  MODE
   ```
5. In queste condizioni, collegare il terminale a coccodrillo, o il puntale remoto alla fase L2 come mostrato nelle Fig. **35** o Fig. **36**.
   Sul display viene visualizzato il messaggio " Tocca L2 " ad indicare l'attesa che lo strumento sia connesso alla fase L2 del sistema in prova. Toccare la parte attiva della fase L2.
   ```
   1 2 3   15/10   –   18:04
   TN
   -   -   -
...
*   `L-N`: Misura dell'impedenza di linea fra il conduttore di fase e il conduttore di neutro. La misura è svolta anche con risoluzione alta (`0.1m`) con accessorio opzionale `IMP57`.
*   `L-L`: Misura dell'impedenza di linea fra due conduttori di fase (`L1-L2` per sistemi Bifase). La misura è svolta anche con risoluzione alta (`0.1m`) con accessorio opzionale `IMP57`.
...
1. Premere il tasto `MENU`, spostare il cursore su ` V%` nel menu principale tramite i tasti freccia (``, ``) e confermare con `ENTER`. Successivamente lo strumento visualizza una videata simile a quella qui riportata a lato.
   ```
      V %   15/10   –   18:04
    V%
   ZL - N  =   -   -   - %
...
2. Utilizzare i tasti ``, `` per selezionare il parametro da modificare e i tasti ``, `` per modificare il valore del parametro:
    ...
    *   `Lim` → Il tasto virtuale permette di impostare il valore limite massimo consentito della caduta di tensione (` V%`) per la linea principale in prova.
    *   `Z><` → Questa posizione permette di eseguire la prima misura di impedenza `Z1` (Offset). In questo caso lo strumento misurerà l'impedenza a monte come punto iniziale della linea principale in prova prendendola come riferimento di partenza.
...
5. Usare i tasti ``, `` e spostare il cursore nella posizione "`Z><`". Premere il tasto `GO/STOP` sullo strumento per avviare il test. La seguente videata è mostrata a display.
   ```
      V %   15/10   –   18:04
    V%
   ZL - L  =   -   -   - %
   ...
   MODE   Inom   Lim.   Z >    <
   ```
6. Usare i tasti ``, `` e spostare il cursore nella posizione "`Z><`". Premere il tasto `GO/STOP` sullo strumento per avviare il test. Il risultato della misurazione Z1 (offset) è mostrata a display sopra la posizione "`Z><`". Se il valore di Z1 (offset) è `<10` il risultato “ OK ” è mostrata a display e salvato automaticamente nel buffer interno.
   ```
      V %   15/10   –   18:04
    V%
   ZL - L  =   -   -   - %
   ...
   MODE   Inom   Lim.   Z >    <
   ```
7. Collegare lo strumento al punto finale della linea principale in prova in accordo alle Fig. **37** o Fig. **38** per misurare l'impedenza `Z2` a fine linea. Notare il valore Z1 (Offset) precedentemente misurato mostrato a display.
8. Utilizzare i tasti ``, `` e spostare il cursore in qualsiasi posizione tranne "`Z><`". Premere il tasto `GO/STOP` sullo strumento per misurare l'impedenza Z2 e completare la misura della caduta di tensione ` V%`. Durante tutta questa fase non scollegare i cavi di misura dello strumento dal sistema in prova.
...
*   Corrente di prova: `>200mA DC` fino a `5` (inclusi puntali di misura)
*   Corrente di prova generata: risoluzione `1mA`, campo `0 ÷ 250mA`
*   Tensione a vuoto: `4 < V0 < 24V DC`
...
*   Corrente di misura nominale: `>1mA` su `1k` x Vnom (50V, 100V, 250V, 1000V), `>2.2mA` con `230k` @ 500V
...
*   `(*) 0.1m` nel campo `0.1  199.9m` (con accessorio opzionale IMP57)
*   Massima corrente di prova: `3.31A (@ 265V)`; `5.71A (@ 457V)`
...
*   Sistemi Monofase (L-N-PE)
    *   Campo tensione L-PE, L-N: `100V  265V` RCD tipo AC, A/F, B/B+ e CCID (`IN ≤100mA`)
    *   `190V  265V` RCD tipo B/B+ (`IN = 300mA`)
    *   Campo tensione N-PE: `<10V`
*   Sistemi Bifase (ritardo fase VL1-PE, VL2-PE = 180° o ritardo fase VL1-PE, VL2-PE = 120°)
    *   Campo tensione L1-PE, L1-L2: `100V  265V` RCD tipo AC, A/F, B/B+ e CCID (`IN ≤100mA`)
    *   Campo tensione L2-PE: `0V  min[(VL1-PE - 100V) e (VL1-L2 - 100V)`, RCD tipo B/B+ (`IN ≤100mA`)
*   Corrente di intervento (`IN`): 5mA, 6mA, 10mA, 20mA, 30mA, 100mA, 300mA, 500mA, 650mA, 1000mA
*   Frequenza: `50/60Hz ±5%`
...
| :----------- | :----------- | :--------------- | :--------------- | :------------------- |
| CCID         | 5mA, 20mA    | `(0.2  1.3) IN` | ` 0.1IN`       | `-0%, +10%IN`       |
| AC, A/F, B/B+ | 6mA,10mA     | `(0.2  1.1) IN` | AC, A/F, B/B+    | `-0%, +5%IN`        |
|              | 30mA  `IN`  300mA |                 |                  |                      |
| AC, A/F      | 500mA  `IN`  650mA |                 |                  |                      |
...
| CCID  |      |      |      |      |     |     |     |     |     |     |     |
| 6mA   |      |      |      |      |     |     |     |     |     |     |     |
| AC    | 999  | 999  | 999  | 999  | 50  | 150 | ✓   | ✓   | 310 | ✓   |     |
| A/F   | 999  | 999  | 999  | 999  | 50  | 150 | ✓   | ✓   | 310 | ✓   |     |
...
*   Correnti di intervento nominali (`IN`): `6mA`
*   Frequenza: `50/60Hz ±5%`

**RCD tipo DD corrente di intervento - (solo RCD tipo Generale)**

| Tipo RCD | `IN` | Campo [mA]        | Risoluzione [mA] | Incertezza           |
| :------- | :---- | :---------------- | :--------------- | :------------------- |
| DD       | 6mA   | `(0.2  1.1) IN` | ` 0.1IN`       | `-0%, +10%IN`       |

**RCD tipo DD tempo di intervento x1 - (solo RCD tipo Generale)**

| Tipo RCD | `IN` | Campo [ms] | Risoluzione [ms] | Incertezza             |
| :------- | :---- | :--------- | :--------------- | :--------------------- |
...
*   `M`: IEC/EN61557-2, BS7671 17th ed., AS/NZS3000/3017
...
| Tipo RCD | IdN [mA]      | `½ In (*)` | `In` | `5 x In` | Note                       |
| :------- | :------------ | :---------- | :---- | :-------- | :------------------------- |
...
(*) Corrente di intervento `½ In`, RCD non deve intervenire.
...
*   Impianti Trifase: `I_BC >= (C_MAX * U_NOM) / (sqrt(3) * Z_L-L)`
*   Impianti Monofase: `I_BC >= (C_MAX * U_NOM) / Z_L-N`

ove: `I_BC` = potere di interruzione della protezione (Breaking Capacity)
...
Tale prescrizione è soddisfatta dalla condizione: `Zs * Ia  Uo`
...
dove:
`ZL-PE` = Impedenza misurata tra Fase e PE.
`ZL-N` = Impedenza misurata tra Fase e Neutro.
`ZLIM` = Valore limite Massimo dell’impedenza funzione del tipo di protezione (Magnetotermico o Fusibile) e dal tempo di intervento della protezione (valore dipendente dalla Nazione di riferimento).
...
Quando siamo in presenza di differenziali da `30mA` (la quasi totalità degli impianti civili) la resistenza di terra massima ammessa è `50/0.03=1666` questo consente di utilizzare anche i metodi semplificati indicati che pur non fornendo un valore estremamente preciso, forniscono un valore sufficientemente approssimato per il calcolo del coordinamento.
...
Verificare che l’impedenza del dispersore a cui sono collegate le masse soddisfi la relazione: `ZE  UL / Id`
...
*   Sistema Trifase → Impedenza Loop F-F: `I_SC_L-L_Min2F > I_a`
*   Sistema Monofase → Impedenza Loop F-N: `I_SC_N-L_Min > I_a`
*   Sistema Monofase → Impedenza Loop F-PE: `I_SC_PE-L_Min > I_a`
...
*   Fase – Fase: `I_SC_L-L_Min2F = (C_MIN * U_NOM) / Z_L-L`
*   Fase – Neutro: `I_SC_N-L_Min = (C_MIN * U_NOM) / Z_L-N`
*   Fase – PE: `I_SC_PE-L_Min = (C_MIN * U_NOM) / Z_L-PE`
...
dove:
`U_L-L` = Tensione fase – fase nominale.
`U_L-N` = Tensione fase – neutro nominale.
`U_L-PE` = Tensione fase – PE nominale.
`Z_L-L` = Impedenza misurata fra fase e fase.
`Z_L-N` = Impedenza misurata fra fase e neutro.
`Z_L-PE` = Impedenza misurata fra fase e PE.
...
Lo strumento esegue il confronto tra il valore calcolato della caduta di tensione massima ` V%` e il limite impostato (tipicamente `4%` in accordo alla normativa CEI 64-8) in base alla seguente relazione:

` V% = 100 * I_NOM * (Z2 - Z1) / V_NOM`
