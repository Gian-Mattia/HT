...
> **ATTENZIONE**
> Per la sicurezza dell'operatore e per evitare di danneggiare lo strumento, seguire le procedure descritte nel presente manuale e leggere con particolare attenzione tutte le note precedute dal simbolo (warning symbol).
...
*   (Dangerous voltage symbol) **Pericolo tensioni pericolose**: rischio di shock elettrici
*   (DC/AC symbol) **Tensione o corrente DC o AC**
*   (Double insulation symbol) **Strumento con doppio isolamento**
*   (Ground symbol) **Riferimento di terra**
*   (Overvoltage category symbol) Il simbolo indica che lo strumento non deve essere connesso a sistemi con tensione nominale concatenata (Fase - Fase) superiore a 605V.
...
> **ATTENZIONE**
> Se durante l'utilizzo compare il simbolo di batteria scarica (battery symbol) sospendere le prove e sostituire le batterie secondo la procedura descritta al § 5.2
...
*   (Phase rotation symbol): Senso ciclico delle fasi a 1 o 2 terminali
...
*   **RCD**: Tempo e corrente di intervento su **RCD** Generali tipo **AC** (AC symbol) e **A/F** (A/F symbol)
...
Il Fattore di Cresta è definito come il rapporto fra il Valore di Picco di un segnale ed il suo Valore Efficace: **CF(G)** = Gp / GRMS
...
Lo strumento è alimentato con 4x1.5V batterie alcaline tipo AA LR06. Quando le batterie sono scariche, il simbolo di batteria scarica "(battery symbol)" viene indicato. Per la sostituzione delle batterie seguire le istruzioni indicate al § 5.2.
...
1.  Mantenendo premuto il tasto **FUNC/HOLD** accendere lo strumento con il tasto **ON/OFF**. Il simbolo "(Auto Power Off symbol)" scompare a display
2.  Alla successiva accensione la funzione sarà automaticamente attivata e il simbolo "(Auto Power Off symbol)" riappare a display
...
3.  Premere i tasti freccia fino a selezionare il valore desiderato (valori possibili sono 0.25, 0.50, 1.00MΩ)
...
6.  Esempio di visualizzazione del valore di resistenza rilevato. Qualora tale valore sia < 40Ω lo strumento emette un segnale acustico
...
4.  Possono essere calibrati cavi che presentino una resistenza fino a 5Ω
...
6.  Qualora il valore misurato durante la fase di calibrazione sia superiore a 5Ω (ex: terminali aperti) lo strumento rimuove il valore dell’offset precedentemente adottato e non visualizza il simbolo **CAL** fino alla successiva calibrazione effettuata con successo.
...
1.  Il fondo scala dello strumento è 39.99kΩ. Qualora il valore della resistenza misurata fosse superiore a tale limite, oppure in caso di puntali aperti od interrotti, lo strumento visualizza la videata a fianco
...
1.  Accendere lo strumento con il tasto **ON/OFF**
2.  Usare i tasti freccia per selezionare la funzione (Phase rotation symbol)
...
> **ATTENZIONE**
> Per il corretto funzionamento della modalità **1W** è necessario che il centro stella della terna trifase in esame sia al potenziale di terra. Negli impianti con neutro isolato come gli impianti **IT** (spesso presenti in ospedali, aeroporti, ecc.) è necessario selezionare la modalità **2W** e collegare il puntale verde al conduttore di neutro (non al conduttore di protezione). In questo genere di impianti la modalità **1W** potrebbe non dare risultati corretti.
...
| :------------------------------- | :-------------------------------------------------------------------------------------------------- | :-------------------------------------------- | :-------------- |
| **OPEN PAIR**                    | COPPIA APERTA Uno od entrambi i cavi appartenenti alla coppia (ex: 1-2) sono interrotti         | (Image placeholder for OPEN PAIR)           | `1 2 4 3 5 6 S 8 7` `1 2 4 3 5 6 S 8 7` |
| **REVERSED PAIR**                | COPPIA ROVESCIATA I cavi appartenenti alla stessa coppia (ex: 1-2) sono scambiati                | (Image placeholder for REVERSED PAIR)       | `1 2 4 3 5 6 S 8 7` `1 2 4 3 5 6 S 8 7` |
| **SHORTED CABLES**               | CAVI CORTOCIRCUITATI Due cavi (ex: 1-8) sono in cortocircuito tra di loro                     | (Image placeholder for SHORTED CABLES)      | `1 2 4 3 5 6 S 8 7` `1 2 4 3 5 6 S 8 7` |
| **TRANSPOSED (CROSSED) PAIRS** | COPPIE SCAMBIATE Due coppie (ex: 1-2 e 7-8) sono scambiate                                  | (Image placeholder for TRANSPOSED PAIRS)    | `1 2 4 3 5 6 S 8 7` `1 2 4 3 5 6 S 8 7` |
| **MISWIRE**                      | ERRORE DI CABLAGGIO Errore generico di cablaggio, come ad esempio due cavi appartenenti a coppie diverse sono scambiati | (Image placeholder for MISWIRE)             | `1 2 4 3 5 6 S 8 7` `1 2 4 3 5 6 S 8 7` |
| **SPLIT PAIRS**                  | COPPIE INCROCIATE La corrispondenza pin a pin è mantenuta, ma fisicamente i cavi delle due coppie sono incrociati | (Image placeholder for SPLIT PAIRS)         | `1 2 4 3 5 6 S 8 7` `1 2 4 3 5 6 S 8 7` |
...
Figura 9: Connessione dei terminali dello strumento
...
6.  Qualora i cavi di misura in uso non siano stati calibrati in precedenza, effettuarne la calibrazione come descritto al sezione 4.12.1.
7.  Collegare i terminali dello strumento ai capi del conduttore di cui si desidera effettuare la prova di continuità (vedere Figura 9).
...
Figura 10: Connessione dei terminali durante la procedura di calibrazione
...
3.  Cortocircuitare tra di loro le estremità dei cavi di misura (vedere Figura 10) prestando attenzione che le parti metalliche dei puntali o dei coccodrilli facciano ben contatto tra di loro.
...
7.  Qualora il valore misurato durante la fase di calibrazione sia superiore a 5 Ω lo strumento interrompe la procedura di calibrazione, rimuove il valore dell’offset precedentemente adottato e non visualizza il simbolo `CAL` fino alla successiva calibrazione effettuata con successo. Lo strumento emette un segnale acustico prolungato che segnala l'esito negativo della calibrazione e visualizza una videata simile a quella a fianco per 2 secondi, quindi visualizza la videata di default relativa alla prova `Ω 0.2A`. Nota: questo metodo è utilizzabile per annullare l’ultima calibrazione effettuata.

### 4.12.2. Situazioni anomale

1.  Nel caso in cui si verifichi la condizione: `R MISURATA - R CALIBRAZIONE < - 0.02 Ω` lo strumento visualizza la videata a fianco ed emette un segnale acustico prolungato a segnalare la situazione anomala.
2.  Se, all’atto della misura, la tensione presente ai terminali è `> 10V`, lo strumento non effettua la prova. Viene emesso un segnale acustico prolungato a segnalare la situazione anomala e viene visualizzata la videata a fianco per 5 s, trascorsi i quali lo strumento visualizza la videata di default relativa alla prova `Ω 0.2A`.
...
Figura 11: Connessione dei terminali dello strumento

1.  Accendere lo strumento con il tasto `ON/OFF`.
2.  Premendo i tasti freccia selezionare la funzione `M Ω`. Selezionare la tensione di prova con il tasto `MODE/PEAK` scegliendo tra i valori `250` o `500V DC`. Impostare il valore di soglia limite minima sulla misura (vedere sezione 4.5) in funzione della normativa considerata (ex: CEI 64 - 8 prevede il valore di 1MΩ con tensione di prova 500VDC).
...
6.  Collegare i terminali dello strumento ai capi dei conduttori dei quali si desidera misurare l’isolamento reciproco (vedere Figura 11).
...
9.  Al termine della prova, nel caso in cui il valore della resistenza rilevata risulti superiore alla soglia limite minima impostata (vedere sezione 4.5) lo strumento emette un doppio segnale acustico che segnala l'esito positivo della prova e visualizza una videata come quella a fianco:
    *   Valore misurato della resistenza
    *   Valore della tensione di prova
10. Al termine della prova, nel caso in cui il valore della resistenza rilevata risulti superiore a 999M Ω, cioè al fondo scala, lo strumento emette un doppio segnale acustico che segnala l'esito positivo della prova e visualizza una videata come quella a fianco.
    Si noti come un valore di isolamento superiore a 999M Ω sia un ottimo valore di isolamento, in genere molto maggiore dei requisiti minimi imposti dalle norme.
    *   Valore misurato della resistenza
    *   Valore della tensione di prova
11. Al termine della prova, nel caso in cui il valore della resistenza rilevata risulti inferiore alla soglia limite minima impostata (vedere sezione 4.5) lo strumento emette un segnale acustico prolungato che segnala l'esito negativo della prova e visualizza una videata come quella a fianco:
    *   Valore misurato della resistenza
    *   Valore della tensione di prova

### 4.13.1. Situazioni anomale

Se, all’atto della misura, la tensione presente ai terminali è maggiore di `10V`, lo strumento non effettua la prova. Viene emesso un segnale acustico prolungato a segnalare la situazione anomala e viene visualizzata la videata a fianco per 5 secondi, trascorsi i quali lo strumento visualizza la videata di default relativa alla prova `M Ω`.
*   Valore della tensione rilevata in ingresso.
...
Figura 12: Connessione dei terminali dello strumento
...
4.  Con il tasto `FUNC HOLD` selezionare il tipo di differenziale tra le opzioni `AC` (~) o `A /F` (⎓) (solo 30mA).
...
6.  Collegare il terminale verde dello strumento al conduttore di protezione (terra) ed il conduttore nero al cavo di fase a valle del differenziale da testare (vedere Figura 12).
...
8.  Al termine della prova, nel caso in cui il valore del tempo di intervento rilevato risulti inferiore a 300ms (40ms per IΔn = 30mA x5), lo strumento emette un doppio segnale acustico che segnala l'esito positivo della prova e visualizza una videata come quella a fianco:
    *   Valore misurato del tempo di intervento
    *   Valore della corrente di prova
9.  Al termine della prova, nel caso in cui il valore del tempo di intervento rilevato risulti superiore a 300ms (40ms per IΔn = 30mA x5), oppure nel caso in cui l’interruttore differenziale non intervenga, lo strumento emette un segnale acustico prolungato che segnala l'esito negativo della prova e visualizza una videata come quella a fianco:
...
1.  Accendere lo strumento con il tasto `ON/OFF`.
2.  Usare i tasti freccia per selezionare la funzione `RCD`.
3.  Con il tasto `FUNC HOLD` selezionare il tipo di differenziale tra le opzioni `AC` (~) o `A /F` (⎓) (solo 30mA) e la misura della corrente di intervento (simbolo “→” a display).
...
7.  Mantenere premuto il tasto `GO` per almeno un secondo per eseguire la misurazione con corrente di dispersione in fase con la semionda positiva della tensione di rete (0°), oppure mantenere premuto il tasto `GO` per almeno un secondo e, quando i trattini sul display iniziano a scomparire, premere nuovamente il tasto `GO` per eseguire la misurazione con corrente di dispersione in fase con la semionda negativa della tensione di rete (180°). Esauriti tutti i trattini lo strumento inizia a generare la corrente progressivamente crescente mantenendo sotto controllo il valore della tensione di contatto.
...
9.  Al termine della prova, nel caso in cui il valore della corrente di intervento rilevato risulti superiore a 33mA, oppure nel caso in cui l’interruttore differenziale non intervenga, lo strumento emette un segnale acustico prolungato che segnala l'esito negativo della prova e visualizza una videata come quella a fianco:
    ...
    *   Valore della corrente di prova
10. A prova terminata lo strumento visualizza alternativamente ogni 2s le videate con il valore della corrente di intervento e del tempo di intervento rilevato nella misura come mostrato nella videata a fianco.
...
Figura 13: Connessione dei terminali dello strumento in prova
...
4.  Selezionando la corrente di prova di `100mA` viene fornito anche il valore della corrente di corto circuito fase terra presunta, calcolato secondo la formula `I CC = UN / ZPE` dove:
...
5.  Inserire il cavo shuko nei terminali di ingresso dello strumento.
6.  Inserire il cavo shuko in una presa di corrente (vedere Figura 13). Nella figura è rappresentato il collegamento alla presa.
...
9.  Qualora sia stata selezionata la corrente di prova `100mA` ed il valore di resistenza di terra risulti inferiore a 1999 Ω, premendo il tasto `FUNC /HOLD` sono visualizzati alternativamente i valori della resistenza globale di terra e della corrente di corto circuito presunta fase terra. Lo strumento inoltre visualizza la corrente con la quale è stata effettuata la misurazione:
    *   Valore misurato della resistenza globale di terra
    *   Il simbolo "HOLD" rimane acceso fino a quando non è permesso effettuare una nuova misura
    *   Corrente utilizzata durante la misura
10. Al termine della prova, nel caso in cui il valore della resistenza di terra risulti superiore a 1999 Ω, lo strumento emette un doppio segnale acustico e visualizza una videata come quella a fianco:
...
3.  Qualora all’atto della misurazione sia rilevata una tensione in ingresso inferiore a `100V` lo strumento non effettua la prova. Viene emesso un segnale acustico prolungato a segnalare la situazione anomala e viene visualizzata la videata a fianco per 5 secondi, trascorsi i quali lo strumento visualizza la videata di default relativa alla prova `Ra`.
    Questa condizione può verificarsi, a titolo di esempio, qualora il cavo nero venga collegato erroneamente al conduttore di neutro anziché al conduttore di fase. Se si utilizza il cavo shuko ruotare la spina e ripetere la prova.
4.  Qualora all’atto della misurazione sia rilevata una tensione di contatto troppo elevata tale da superare il limite normativo di `50V`, lo strumento non effettua la prova. Viene emesso un segnale acustico prolungato a segnalare la situazione anomala e viene visualizzata la videata a fianco per 5 secondi trascorsi i quali lo strumento visualizza la videata di default relativa alla prova `Ra`.
...
Figura 14: Connessione dei terminali dello strumento in prova AUTO

1.  Accendere lo strumento con il tasto `ON/OFF`.
2.  Usare i tasti freccia per selezionare la funzione `AUTO`.
3.  I tasti `MODE PEAK` e `FUNC HOLD` non sono attivi per l’impostazione dei parametri di questa funzione. Per il limite minimo della resistenza di isolamento e per la selezione della misura del tempo o della corrente di intervento lo strumento considera sempre le opzioni presenti nelle funzioni `M Ω` (vedere sezione 4.13) e `RCD` (vedere sezione 4.14).
...
5.  Collegare il terminale verde dello strumento al conduttore di protezione (terra) ed il conduttore nero al cavo di fase (vedere Figura 14) o inserire la spina shuko nella presa.
6.  Mantenere premuto il tasto `GO` per almeno un secondo, lo strumento esegue in sequenza e senza la necessità di intervento da parte dell’operatore le seguenti misurazioni: `Ra` (15mA), `RCD` (tempo o corrente di intervento), `M Ω` (fra fase e terra).
...
8.  Al termine della prova `Ra`, nel caso in cui il valore della resistenza di terra risulti inferiore a `50V/IΔn` lo strumento visualizza per 5 secondi una videata come quella a fianco, quindi passa all’esecuzione della misurazione successiva. Si veda il sezione 4.15 per maggiori dettagli o per informazioni riguardo l’eventuale esito negativo della prova o situazioni anomale:
    *   Valore misurato della resistenza globale di terra
...
10. Al termine della prova, nel caso in cui sia stata selezionata la misura della corrente di intervento, il valore rilevato risulti inferiore a `30mA`, lo strumento visualizza per 5 secondi una videata come quella a fianco, quindi passa alla videata successiva. Per maggiori dettagli, per informazioni riguardo l’eventuale esito negativo della prova o situazioni anomale, si veda il sezione 4.14:
    ...
    *   Valore della corrente di prova
11. Al termine della prova `M Ω`, nel caso in cui il valore della resistenza rilevata risulti superiore alla soglia limite minima impostata (vedere sezione 4.5) lo strumento visualizza per 5 secondi una videata come quella a fianco, quindi passa alla videata successiva. Per maggiori dettagli, per informazioni riguardo l’eventuale esito negativo della prova o situazioni anomale, si veda il sezione 4.13:
    *   Valore misurato della resistenza
    *   Valore della tensione di prova
12. Al termine della prova AUTO, nel caso in cui tutte le prove abbiano dato esito positivo, lo strumento emette un doppio segnale acustico a segnalare l'esito positivo e visualizza una videata come quella a fianco. Per visualizzare i risultati parziali premere il tasto `FUNC HOLD`, gli esiti delle singole prove si presentano ciclicamente ad ogni nuova pressione del medesimo tasto. Nella misura della corrente di intervento lo strumento visualizza alternativamente ogni 2s le videate con il valore della corrente di intervento e del tempo di intervento rilevato nella misura.
...
4.  Rimuovere dal pacco batterie tutte le batterie, sostituirle solo con batterie tutte nuove e tutte dello stesso tipo (vedere sezione 6.2) rispettando le polarità indicate.
...
| :---------------- | :---------- | :------------------------- | :--------------------------- | :--------------------------- | :------------------- |
| 1.0 ÷ 999.9mV     | 0.1mV       | ± (0.5 % lett. +2 cifre)   | ± (1.0 % lett. +2 cifre)     | ± (2.0 % lett. +2 cifre)     | 1M Ω                 |
| 1.000 ÷ 9.999V    | 0.001V      |                            |                              |                              |                      |
| 10.00 ÷ 99.99V    | 0.01V       |                            |                              |                              |                      |
| 100.0 ÷ 605.0V    | 0.1V        |                            |                              |                              |                      |
...
| Campo             | Risoluzione | Incertezza DC              | Incertezza (30 ÷ 70Hz)       | Incertezza (71 ÷ 400Hz)      | Imp. ingresso | Protezione sovraccarichi |
| :---------------- | :---------- | :------------------------- | :--------------------------- | :--------------------------- | :------------ | :----------------------- |
| 1.0 ÷ 999.9mV     | 0.1mV       | ± (0.5 % lett. +2 cifre)   | ± (1.0 % lett. +2 cifre)     | ± (2.0 % lett. +2 cifre)     | 1M Ω          | 605V AC max RMS          |
| 1.000 ÷ 1.200V    | 0.001V      |                            |                              |                              |               |                          |
...
| :---------------- | :---------- | :------------------------- | :------------------- |
| 30.0 ÷ 199.9Hz    | 0.1Hz       | ± (0.5 % lettura + 2 cifre) | 1M Ω                 |
| 200 ÷ 400Hz       | 1Hz         |                            |                      |
...
| :---------------- | :---------- | :------------------------ | :----- | :-------------------------------- |
| 0.00 ÷ 39.99 Ω    | 0.01 Ω      | ± (1% lettura + 5 cifre)  | R<40 Ω | 605V AC max RMS per 1 minuto      |
| 40.0 ÷ 399.9 Ω    | 0.1 Ω       |                           |        |                                   |
| 400 ÷ 3999 Ω      | 1 Ω         |                           |        |                                   |
| 4.00 ÷ 39.99k Ω   | 10 Ω        |                           |        |                                   |
...
| Campo             | Risoluzione | Incertezza                 | Protezione contro i sovraccarichi |
| :---------------- | :---------- | :------------------------- | :-------------------------------- |
| 0.00 ÷ 19.99 Ω    | 0.01 Ω      | ± (5.0% lettura + 3 cifre) | 605V max RMS                      |
| 20.0 ÷ 99.9 Ω     | 0.1         |                            |                                   |

Corrente di Prova: >200mA DC fino a 5 Ω (resistenza dei cavi di misura compresa); Risoluzione misura corrente: 1mA.
Tensione a Vuoto: 4 < V0 < 24V.

### M Ω : Resistenza di isolamento 250, 500VDC

| Campo             | Risoluzione | Incertezza                 | Protezione contro i sovraccarichi |
| :---------------- | :---------- | :------------------------- | :-------------------------------- |
| 0.00 ÷ 19.99M Ω   | 0.01M Ω     | ± (5.0% lettura + 2 cifre) | 605V max RMS                      |
| 20.0 ÷ 199.9M Ω   | 0.1M Ω      | ± (5.0% lettura + 2 cifre) |                                   |
| 200 ÷ 999M Ω      | (*) 1M Ω    | ± (10.0% lettura + 2 cifre)|                                   |

(*) Per tensione di prova 500VDC. Per tensione di prova 250V il campo è: 200 ÷ 499M Ω.
...
Corrente di misura nominale: 1mA @ 1K Ω x V (1mA @ 500K Ω).
...
Tipo differenziale: AC (~), A /F (⎓), Generali (G).
Tensione Fase – Terra / Fase – Neutro: 100 ÷ 265V.
Correnti di prova: 30mA, 30mA x 5, 100mA, 300mA (Tipo AC), 30mA (Tipo A /F).
Frequenza: 50Hz ± 0.5Hz / 60Hz ± 0.5Hz.

### RCD: Corrente di intervento dei differenziali

| Tipo RCD            | I ΔN   | Campo I ΔN [mA] | Risoluzione | Incertezza            |
| :------------------ | :----- | :-------------- | :---------- | :-------------------- |
| AC, A /F (Generali) | 30mA   | 6.0 ÷ 33.0      | 0.5mA       | - 0%, +10%I ΔN        |
...
| :---------------- | :-------------- | :---------- | :------------------------ | :-------------------------------- |
| 15mA              | 1 ÷ 1999 Ω      | 1 Ω         | ± (5% lettura + 2 cifre)  | 605V max RMS                      |
| 100mA             | 0.1 ÷ 199.9 Ω   | 0.1 Ω       | ± (5% lettura + 3 cifre)  |                                   |
...
*   **Durata batterie:**
    ...
    *   LAN: > 1000 prove
    *   Ω 0.2A: > 1000 prove @ 1 Ω
    *   M Ω : > 1000 prove @ 480k Ω (500VCC)
    *   RCD: > 1000 prove
    *   Ra : > 1000 prove
    *   AUTO: > 1000 prove
