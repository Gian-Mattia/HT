...
<!-- Chunk: Pages 1-31 -->
© Copyright HT ITALIA 2019 Versione IT 2.00 - xx/xx/2019 VEGA78 Manuale d'uso

# INDICE
1.  [PRECAUZIONI E MISURE DI SICUREZZA](#1-precauzioni-e-misure-di-sicurezza)
    1.1. [Generalità](#11-generalità)
    1.2. [Istruzioni preliminari](#12-istruzioni-preliminari)
    1.3. [Durante l’utilizzo](#13-durante-lutilizzo)
    1.4. [Dopo l’utilizzo](#14-dopo-lutilizzo)
2.  [DESCRIZIONE GENERALE](#2-descrizione-generale)
    2.1. [Introduzione](#21-introduzione)
    2.2. [Funzionalità dello strumento](#22-funzionalita-dello-strumento)
    2.3. [Videata Iniziale](#23-videata-iniziale)
...
4.  [ISTRUZIONI OPERATIVE](#4-istruzioni-operative)
    ...
    4.4. [Reset dello strumento](#44-reset-dello-strumento)
5.  [MENU GENERALE](#5-menu-generale)
    5.1. [Sezione Impostazioni Generali](#51-sezione-impostazioni-generali)
        5.1.1. [Regolazione Lingua di sistema](#511-regolazione-lingua-di-sistema)
        5.1.2. [Regolazione Data/Ora di sistema](#512-regolazione-dataora-di-sistema)
        5.1.3. [Regolazione Luminosità del display](#513-regolazione-luminosita-del-display)
        5.1.4. [Impostazione Password di protezione](#514-impostazione-password-di-protezione)
        5.1.5. [Impostazione Suoni tastiera](#515-impostazione-suoni-tastiera)
        5.1.6. [Impostazione Autospegnimento](#516-impostazione-autospegnimento)
        5.1.7. [Impostazione Tipo Memoria](#517-impostazione-tipo-memoria)
        5.1.8. [Calibrazione display “touch screen”](#518-calibrazione-display-touch-screen)
    5.2. [Sezione Visualizzazione Misure](#52-sezione-visualizzazione-misure)
        5.2.1. [Visualizzazione valori numerici](#521-visualizzazione-valori-numerici)
        5.2.2. [Videate forme d’onda segnali SCOPE](#522-videate-forme-donda-segnali-scope)
        5.2.3. [Videate analisi armonica ARM](#523-videate-analisi-armonica-arm)
        5.2.4. [Videate diagramma vettoriale](#524-videate-diagramma-vettoriale)
    5.3. [Sezione Impostazione Analizzatore](#53-sezione-impostazione-analizzatore)
        5.3.1. [Videata Configurazione Analizzatore](#531-videata-configurazione-analizzatore)
            5.3.1.1. [Impostazione frequenza del sistema](#5311-impostazione-frequenza-del-sistema)
            5.3.1.2. [Impostazione del tipo di pinza](#5312-impostazione-del-tipo-di-pinza)
            5.3.1.3. [Impostazione del fondo scala della pinza](#5313-impostazione-del-fondo-scala-della-pinza)
            5.3.1.4. [Impostazione del rapporto di trasformazione dei TV](#5314-impostazione-del-rapporto-di-trasformazione-dei-tv)
        5.3.2. [Videata Impostazioni Avanzate](#532-videata-impostazioni-avanzate)
            5.3.2.1. [Opzione Zoom Grafici](#5321-opzione-zoom-grafici)
            5.3.2.2. [Opzione Armoniche visualizzate](#5322-opzione-armoniche-visualizzate)
            5.3.2.3. [Opzione valori delle armoniche](#5323-opzione-valori-delle-armoniche)
            5.3.2.4. [Opzione Zoom rispetto alla prima armonica](#5324-opzione-zoom-rispetto-alla-prima-armonica)
            5.3.2.5. [Opzione Valori Medi](#5325-opzione-valori-medi)
    5.4. [Sezione Impostazione Registrazione](#54-sezione-impostazione-registrazione)
        5.4.1. [Videata Impostazioni Registratore](#541-videata-impostazioni-registratore)
        5.4.2. [Voce Commenti](#542-voce-commenti)
            5.4.2.1. [Uso della tastiera virtuale](#5421-uso-della-tastiera-virtuale)
        5.4.3. [Voci Start e Stop](#543-voci-start-e-stop)
        5.4.4. [Voce Periodo Integrazione](#544-voce-periodo-integrazione)
        5.4.5. [Voce Cogenerazione](#545-voce-cogenerazione)
        5.4.6. [Voce Parametri Generali](#546-voce-parametri-generali)
            5.4.6.1. [Descrizione sottolivelli voce Parametri Generali](#5461-descrizione-sottolivelli-voce-parametri-generali)
            5.4.6.2. [Descrizione sottolivelli voce Armoniche](#5462-descrizione-sottolivelli-voce-armoniche)
        5.4.7. [Voce Anomalie di Tensione](#547-voce-anomalie-di-tensione)
        5.4.8. [Voce Dissimmetria](#548-voce-dissimmetria)
        5.4.9. [Configurazioni predefinite](#549-configurazioni-predefinite)
        5.4.10. [Avvio di una registrazione](#5410-avvio-di-una-registrazione)
            5.4.10.1. [Avvio automatico della registrazione](#54101-avvio-automatico-della-registrazione)
    5.4.11. [Durante una registrazione](#5411-durante-una-registrazione)
    5.5. [Sezione gestione dati memorizzati](#55-sezione-gestione-dati-memorizzati)
        5.5.1. [Analisi registrazione (tipo Reg)](#551-analisi-registrazione-tipo-reg)
            5.5.1.1. [Informazioni Registrazione](#5511-informazioni-registrazione)
            5.5.1.2. [Grafico Registrazione](#5512-grafico-registrazione)
            5.5.1.3. [Anomalie](#5513-anomalie)
            5.5.1.4. [Analisi energia consumata](#5514-analisi-energia-consumata)
            5.5.1.5. [Analisi energia prodotta](#5515-analisi-energia-prodotta)
        5.5.2. [Analisi registrazione (tipo Istant)](#552-analisi-registrazione-tipo-istant)
            5.5.2.1. [Informazioni Registrazione](#5521-informazioni-registrazione)
            5.5.2.2. [Grafico](#5522-grafico)
            5.5.2.3. [Analisi Armonica](#5523-analisi-armonica)
            5.5.2.4. [Vettori](#5524-vettori)
            5.5.2.5. [Misure](#5525-misure)
        5.5.3. [Trasferimento di registrazioni su Pen Driver USB esterno](#553-trasferimento-di-registrazioni-su-pen-driver-usb-esterno)
        5.5.4. [Salvataggio registrazioni su Compact Flash esterna](#554-salvataggio-registrazioni-su-compact-flash-esterna)
    5.6. [Sezione Informazioni strumento](#56-sezione-informazioni-strumento)
6.  [COLLEGAMENTO DELLO STRUMENTO AD UN PC (WINXP)](#6-collegamento-dello-strumento-ad-un-pc-winxp)
7.  [PROCEDURE DI MISURA](#7-procedure-di-misura)
    7.1. [Uso dello strumento in un Sistema Monofase](#71-uso-dello-strumento-in-un-sistema-monofase)
    7.2. [Uso dello strumento in un Sistema Trifase 4 - fili](#72-uso-dello-strumento-in-un-sistema-trifase-4---fili)
    7.3. [Uso dello strumento in un Sistema Trifase 3 - fili](#73-uso-dello-strumento-in-un-sistema-trifase-3---fili)
    7.4. [Uso dello strumento in un Sistema Trifase 3 - fili ARON](#74-uso-dello-strumento-in-un-sistema-trifase-3---fili-aron)
    7.5. [Uso dello strumento in un Sistema 4 - fili 3F HL (sistemi USA)](#75-uso-dello-strumento-in-un-sistema-4---fili-3f-hl-sistemi-usa)
    7.6. [Uso dello strumento in un Sistema 3 - fili 3F DA (sistemi USA)](#76-uso-dello-strumento-in-un-sistema-3---fili-3f-da-sistemi-usa)
    7.7. [Uso dello strumento in un Sistema 3 - fili 3F 2e (sistemi USA)](#77-uso-dello-strumento-in-un-sistema-3---fili-3f-2e-sistemi-usa)
    7.8. [Uso dello strumento in un Sistema 3 - fili 3F YA (sistemi USA)](#78-uso-dello-strumento-in-un-sistema-3---fili-3f-ya-sistemi-usa)
    7.9. [Uso dello strumento in un Sistema 3 - fili 1F PC (sistemi USA)](#79-uso-dello-strumento-in-un-sistema-3---fili-1f-pc-sistemi-usa)
8.  [MANUTENZIONE](#8-manutenzione)
    8.1. [Generalità](#81-generalita)
    8.2. [Situazioni relative alla batteria interna](#82-situazioni-relative-alla-batteria-interna)
    8.3. [Pulizia dello strumento](#83-pulizia-dello-strumento)
    8.4. [Fine vita](#84-fine-vita)
9.  [SPECIFICHE TECNICHE](#9-specifiche-tecniche)
    9.1. [Caratteristiche Tecniche](#91-caratteristiche-tecniche)
    9.2. [Caratteristiche Generali](#92-caratteristiche-generali)
    9.3. [Ambiente](#93-ambiente)
        9.3.1. [Condizioni ambientali di utilizzo](#931-condizioni-ambientali-di-utilizzo)
    9.4. [Accessori](#94-accessori)
10. [APPENDICE – CENNI TEORICI](#10-appendice--cenni-teorici)
    10.1. [Anomalie di tensione](#101-anomalie-di-tensione)
    10.2. [Armoniche di tensione e corrente](#102-armoniche-di-tensione-e-corrente)
        10.2.1. [Teoria](#1021-teoria)
        10.2.2. [Valori limite per le Armoniche di tensione](#1022-valori-limite-per-le-armoniche-di-tensione)
        10.2.3. [Cause della Presenza di Armoniche](#1023-cause-della-presenza-di-armoniche)
        10.2.4. [Conseguenza della Presenza di Armoniche](#1024-conseguenza-della-presenza-di-armoniche)
    10.3. [Dissimmetria delle tensioni di alimentazione](#103-dissimmetria-delle-tensioni-di-alimentazione)
    10.4. [Definizioni di Potenza e Fattore di Potenza](#104-definizioni-di-potenza-e-fattore-di-potenza)
        10.4.1. [Convenzioni sulle Potenze e Fattori di Potenza](#1041-convenzioni-sulle-potenze-e-fattori-di-potenza)
        10.4.2. [Inserzione ARON](#1042-inserzione-aron)
    10.5. [Cenni sul Metodo di Misura](#105-cenni-sul-metodo-di-misura)
        10.5.1. [Uso del periodo di integrazione](#1051-uso-del-periodo-di-integrazione)
        10.5.2. [Calcolo del fattore di potenza](#1052-calcolo-del-fattore-di-potenza)
...
## 1.1. GENERALITÀ
Lo strumento è stato progettato in conformità alla direttiva IEC/EN61010-1, relativa agli strumenti di misura elettronici. Per la Sua sicurezza e per evitare di danneggiare lo strumento, La preghiamo di seguire le procedure descritte nel presente manuale e di leggere con particolare attenzione tutte le note precedute dal simbolo ![Warning Symbol].
...
> **Doppio isolamento**
> Le tensioni indicate a sinistra di questo simbolo ![Voltage Reference Symbol] si intendono riferite verso terra
...
## 2.1. INTRODUZIONE
Lo strumento **VEGA78** consente un approccio totalmente nuovo al mondo delle misure su impianti elettrici in quanto la combinazione di strumenti a microprocessore e la tecnologia Windows CE consente di analizzare una enorme mole di dati con una semplicità ed una velocità impossibili da ottenere con qualunque altro sistema.

## 2.2. FUNZIONALITÀ DELLO STRUMENTO
Lo strumento consente di effettuare le seguenti operazioni:
*   Visualizzazione in tempo reale dei valori di ogni grandezze elettriche di un impianto Monofase e Trifase 3-fili o 4-fili, Analisi Armonica di tensioni e correnti fino alla 64^a^ armonica, Anomalie di Tensione (buchi e picchi) con risoluzione 10ms, visualizzazione dei parametri della dissimmetria sulle tensioni
...
9.  Tasto **HELP**
10. Tasto **ESC**
11. Uscita per connessione a memoria Flash esterna
...
*   **Tasto GO/STOP**: Avvio/arresto manuale della registrazione (vedere il § 5.4.10).
...
L’operazione di **RESET NON** cancella il contenuto della memoria dello strumento.
...
In questa sezione lo strumento mostra i valori delle letture eseguite in tempo reale sui canali di ingresso e internamente calcolate relative a tensioni, correnti AC TRMS, globalità dei parametri elettrici per ogni singola fase e totali, dissimmetria sulle tensioni, visualizzazione forme d’onda di tensioni/correnti per ogni fase e totali, visualizzazione delle Armoniche di tensione e corrente fino alla 64^a^ componente per ogni singola fase e totali, visualizzazione diagramma vettoriale in cui sono rappresentate le tensioni e le correnti con i rispettivi angoli di sfasamento al fine di stabilire la natura dei carichi.
...
*   **V23** → Tensione Tensione Fase L2 - Fase L3
...
Fig. 19: Pagina valori numerici per sistema trifase 4 fili
...
Fig. 24: Pagina Energia Consumata
...
Fig. 25: Pagina Energia Prodotta
...
Fig. 27: Videata forme d’onda delle tensioni per sistema trifase 4-fili
...
Fig. 28: Videata forme d’onda delle correnti per sistema trifase 4-fili

*   Le forme d’onda dei segnali sulla fase **L1** con i rispettivi valori efficaci:

Fig. 29: Videata forme d’onda tensione e corrente fase L1 per sistemi trifase 4-fili

*   Le forme d’onda dei segnali sulla fase **L2** con i rispettivi valori efficaci:

Fig. 30: Videata forme d’onda tensione e corrente fase L2 per sistemi trifase 4-fili

*   Le forme d’onda dei segnali sulla fase **L3** con i rispettivi valori efficaci:

Fig. 31: Videata forme d’onda tensione e corrente fase L3 per sistemi trifase 4-fili
...
Fig. 32: Analisi armonica tensioni in valore percentuale per sistemi trifase 4-fili
...
Fig. 33: Analisi armonica correnti in valore percentuale per sistemi trifase 4-fili

*   Passare alle videate relative alle tabelle dei valori numerici delle armoniche di tensione e corrente fino alla 64^a^ sia in valore percentuale sia in valore assoluto (vedere il § 5.3.2.3) premendo il tasto **F3** (o la voce “**TABELLA**” a display), come mostrato nelle figure seguenti:

Fig. 34: Analisi armonica numerica percentuale tensioni per sistemi 4-fili

Fig. 35: Analisi armonica numerica percentuale correnti in valore per sistemi 4-fili

Premere il tasto **F3** per tornare alla visualizzazione grafica e il tasto **F2** per passare alle videate di tensioni e correnti. Premere il tasto **F4** o i tasti freccia alto o basso (o la voce “**ARM PAG**” a display) per passare alle videate relative alle armoniche di ordine superiore fino alla 64^a^.
*   I valori delle armoniche delle singole tensioni **V1**, **V2**, **V3** e **Vn** e delle singole correnti **I1**, **I2**, **I3** e **In** con rispettivi valori della Distorsione Armonica Totale (**THD%**) sia sotto forma di grafico a istogramma, sia sotto forma di tabella numerica, in valore percentuale o assoluta in funzione dell’impostazione desiderata che sono visualizzati in quattro pagine consecutive e ciclicamente selezionabili. In Fig. 36 e Fig. 37 sono riportati, come esempio, le situazioni relative ai valori della fase L1 rispettivamente per le tensioni e per le correnti per sistemi 4-fili:

Fig. 36: Analisi armonica tensione V1 in valore percentuale e assoluto per sistemi 4-fili

Fig. 37: Analisi armonica Corrente I1 in valore percentuale e assoluto per sistema 4-fili
...
*   Il diagramma vettoriale totale degli sfasamenti tra le tensioni **V1**, **V2**, **V3** e tra **V1-I1**, **V2-I2**, **V3-I3** oltre all’indicazione dei valori percentuali dei parametri “**INV%**” e “**OMO%**” (vedere il § 10.3), come di seguito mostrato:

Fig. 38: Diagramma vettoriale totale per sistemi 4-fili

*   Il diagramma vettoriale delle singole tensioni come riportato nelle figure seguenti:

Fig. 39: Diagramma vettoriale tensioni per sistemi 4-fili

*   Il diagramma vettoriale delle correnti. La Fig. 40 mostra il caso di sistemi 4-fili:

Fig. 40: Diagramma vettoriale correnti per sistemi 4-fili

*   Il diagramma vettoriale di tensione e corrente relativo ad ogni singola fase:

Fig. 41: Diagramma vettoriale tensione - corrente fase L1 per sistemi 4-fili
...
Fig. 42: Videata MENU GENERALE - Impostazione Analizzatore
...
Fig. 43: Videata configurazione analizzatore per sistema Trifase 4-fili

Fig. 44: Videata configurazione analizzatore per sistema Trifase 3-fili

Fig. 45: Videata configurazione analizzatore per sistema Trifase ARON

Fig. 46: Videata configurazione analizzatore per sistema Monofase

Fig. 47: Videata configurazione analizzatore per sistema Bifase 3-fili 1F PC

Fig. 48: Videata configurazione analizzatore per sistema Bifase 3-fili 3F 2E

Fig. 49: Videata configurazione analizzatore per sistema Trifase 3-fili 3F DA

Fig. 50: Videata configurazione analizzatore per sistema Trifase 4-fili 3F HL

Fig. 51: Videata configurazione analizzatore per sistema Bifase 3-fili 3F YA
...
Fig. 52: Videata Impostazioni Avanzate
...
Questa opzione consente di selezionare un fondo scala personalizzato nella visualizzazione delle forme d’onda dei segnali *sulle singole fasi* (vedere le Fig. 29, Fig. 30, Fig. 31) al fine di migliorare la risoluzione di lettura.
...
Il risultato è mostrato nella Pagina `9/9` dei valori numerici all’interno della sezione Visualizzazione Misure (vedere Fig. 26). Posizionare il cursore sulla voce “**Valori Medi**” utilizzando i tasti freccia alto o basso in modo da evidenziarla con sfondo blu.
...
Figura 53: Videata MENU GENERALE - Impostazione Registrazione
...
Figura 54: Videata Impostazioni Registratore
...
*   Funzione `PREDEF.`: Usata per aprire la sezione delle configurazioni predefinite (vedere il § 5.4.9).
...
Figura 55: Videata tastiera virtuale
...
Queste voci permettono di definire il modo di attivazione e disattivazione delle registrazioni effettuabili con lo strumento (vedere il § 5.4.10). Le opzioni possibili sono:
...
La configurazione di default è sempre il modo Manuale e i flags di tali voci non sono deselezionabili. Per passare al modo automatico (vedere il § 5.4.10.1) operare come segue:
...
Questa voce consente di impostare il periodo di integrazione (vedere il § 10.5.1) cioè l’intervallo temporale tra due registrazioni consecutive nell’ambito della intera campagna di misura. Il flag di tale voce è sempre attivo e non deselezionabile.
...
La selezione di questa voce, eseguita sia usando i tasti freccia alto o basso o spuntando direttamente il flag a display, inserisce nell’elenco dei parametri registrabili i valori delle potenze e delle energie generate (cogenerazione) (vedere il § 10.4.1).
...
Figura 58: Sezione Parametri Generali: troppi parametri selezionati

Nella Fig. 58 è possibile notare che la selezione dei parametri di analisi armonica comporta un numero di parametri oltre il massimo consentito di 251 e questo genera la condizione di errore. Procedere in tal caso alla deselezione dei parametri per uscire da tale situazione di errore:
...
Figura 59: Sottolivello Parametri Generali – Sistema trifase 4-fili
...
Tabella 2: Parametri selezionabili per sistema Trifase 4-fili
...
Figura 60: Sottolivello Armoniche – selezione iniziale parametri
...
Figura 61: Sottolivello Armoniche – selezione Armoniche Dispari

Anche in questo caso è possibile selezionare/deselezionare ogni singola armonica usando i tasti freccia alto o basso o direttamente sul display, premendo il tasto `F3` (o la voce `SEL` a display). Il numero dei parametri selezionati e l’autonomia di registrazione si aggiornano automaticamente a display (vedere il § 10.2).
Premere i tasti `SAVE` o `ENTER` (o la `smart icon`) per salvare ogni selezione eseguita. In tale situazione lo strumento presenta una finestra di conferma del salvataggio. Confermare con “Ok” in tale finestra. Per abbandonare le modifiche effettuate o per uscire senza salvare, premere il tasto `ESC` (o la `smart icon`).
...
Figura 62: Videata nessuna corrente selezionata

Nel caso in cui lo strumento mostri la situazione di errore di Fig. 62 selezionare la voce relativa alle “Correnti” nel sottolivello “Parametri Generali” (vedere il § 5.4.6.1).

Figura 63: Videata nessuna tensione selezionata

Nel caso in cui lo strumento mostri la situazione di errore di Fig. 63 selezionare la voce relativa alle “Tensioni” nel sottolivello “Parametri Generali” (vedere il § 5.4.6.1).
...
Figura 64: Videata Anomalie di Tensione
...
Figura 65: Videata Anomalie di Tensione - impostazione parametri
...
### 5.4.8. Voce Dissimmetria

La selezione di questa voce, eseguita sia usando i tasti freccia alto o basso o spuntando direttamente il flag a display, inserisce nell’elenco delle grandezze per la registrazione il valore dei parametri `INV%` e `OMO%` indicativi della dissimmetria delle tensioni in ingresso rispettivamente della terna inversa e della terna omopolare (vedere § 10.3). La voce “Dissimmetria” non compare in caso di selezione di sistema Monofase.

### 5.4.9. Configurazioni predefinite
...
Lo strumento consente inoltre di definire *fino a 14 configurazioni personalizzabili*, salvabili e richiamabili dall’utente in ogni momento. La selezione di queste configurazioni imposta automaticamente sullo strumento esclusivamente i parametri necessari per l’esecuzione della registrazione o quelli scelti dall’utente. Le configurazioni predefinite sono:

1.  **DEFAULT**: impostazione dei parametri all’uscita di fabbrica dello strumento.
2.  **ANOMALIE DI TENSIONE**: impostazione dei parametri per la qualità di rete relativamente alle anomalie di tensione (buchi, picchi, interruzioni – vedere il § 10.1).
3.  **ARMONICHE**: impostazione dei parametri analisi armonica per tensioni e correnti (vedere il § 10.2).
4.  **POTENZA & ENERGIA**: impostazione dei parametri relativi alle misure di Potenze e Energie (vedere il § 10.4).
...
Figura 66: Videata configurazioni predefinite
...
Tabella 3: Elenco parametri configurazione predefinita Anomalie Tensione
...
|                              | Valori Medi              |                            |                             |
| Impostazione Registrazione   | Commenti                 | ANOMALIE TENSIONE          | Ogni sistema                |
...
Tabella 4: Elenco parametri configurazione predefinita Armoniche
...
Tabella 5: Elenco parametri configurazione predefinita Potenze & Energie
...
Tabella 6: Parametri impostati nella configurazione di DEFAULT
...
> **ATTENZIONE**
> La configurazione di `DEFAULT` definisce le selezioni dei parametri con cui lo strumento esce dalla fabbrica come riportato in Tabella 6. Ogni successiva modifica operata e salvata dall’utente nella sezione “Impostazione Analizzatore” modifica tale configurazione con le nuove selezioni che possono differire da quelle iniziali.

### 5.4.10. Avvio di una registrazione

Lo strumento consente l’avvio di una registrazione in modo MANUALE o AUTOMATICO (vedere il § 5.4.3) a partire dalla pressione del tasto `GO/STOP`.
...
Figura 68: Esempi di videate per attivazione registrazione

Premere il tasto `ESC` (o la `smart icon`) sullo strumento fino a riportarsi alle condizioni come nell’esempio di Fig. 68 prima di attivare una registrazione. L’avvio della registrazione è eseguita nei modi seguenti:
...
Figura 69: Strumento in attesa

Figura 70: Registrazione in corso

Premere nuovamente il tasto `GO/STOP` per terminare in qualunque momento la registrazione in corso. L’icona evidenziata nella Fig. 70 scompare a display.
...
Prima di effettuare l'avvio della registrazione una valutazione preliminare in tempo reale della situazione dell'impianto è opportuna in modo da decidere cosa registrare e impostare quindi coerentemente lo strumento sfruttando eventualmente le configurazioni predefinite (vedere il § 5.4.9).
...
Figura 71: Finestra messaggio troppi parametri selezionati
...
Figura 72: Finestra messaggi promemoria errori

Il significato delle voci presenti nella finestra è il seguente:

Tabella 7: Descrizione errori prima di avvio della registrazione
...
Eseguire le modifiche alla programmazione dello strumento in base al numero degli errori visualizzati nella Fig. 72 e premere nuovamente il tasto `GO/STOP` per avviare la registrazione verificando eventuali errori residui nella finestra messaggi.
...
Siccome il valore predefinito del periodo di integrazione (vedere il § 10.5.1) è impostato a `15min` lo strumento accumulerà dati nella memoria temporanea per tale tempo. Trascorso tale periodo di tempo lo strumento elaborerà i risultati memorizzati nella memoria temporanea e salverà nella memoria definitiva dello strumento la prima serie di valori relativi alla registrazione.
...
#### 5.4.10.1. Avvio automatico della registrazione

Il modo di avvio automatico di una registrazione prevede la definizione di una Data/Ora coerente con quella di sistema, all’interno della sezione “Impostazione Registrazione” (vedere il § 5.4). Lo strumento presenta la videata seguente:

Figura 73: Data/Ora corretta

Figura 74: Data/Ora non corretta

1.  Usare i tasti freccia destro o sinistro per muovere il cursore sui campi corrispondenti alla Data/Ora.
2.  Premere i tasti `F3` o `F4` (oppure `MOD(+)` o `MOD(-) ` a display) per incrementare o decrementare di una unità il valore delle cifre fino ad impostare il valore desiderato coerente con la data di sistema (vedere la Fig. 73).
3.  Premere i tasti `SAVE` o `ENTER` (o la `smart icon`) per salvare ogni impostazione eseguita. In caso di errata impostazione lo strumento fornisce la videata di errore riportata in Fig. 74. Premere il tasto `F4` (oppure `MODIFICA` a display) e ripetere le operazioni del punto 2 inserendo una data/ora coerente. In queste condizioni il tasto `SAVE` non ha alcun effetto fino al ripristino della corretta operazione.

### 5.4.11. Durante una registrazione
...
Figura 75: Videata Gestione Dati Memorizzati – Registrazione in corso

1.  Usare i tasti freccia oppure selezionare l’icona “Gestione Dati Memorizzati”
...
Figura 76: Videata informazioni registrazione in corso

3.  Nella videata di Fig. 76 sono riportate le seguenti informazioni:
    *   Autonomia di registrazione dello strumento con le selezioni effettuate in Giorni/Ore.
    *   Commenti inseriti nel campo “Impostazione Registrazione” (vedere il § 5.4.2).
    *   Tipo di avvio della registrazione.
...
Figura 77: MENU GENERALE - Gestione Dati Memorizzati
...
*   **Tipo Istant**: Campionamenti istantanei dei valori a display (numerici, forme d’onda, armoniche, ecc…) effettuati premendo il tasto `SAVE`.

Ogni riga della videata “Gestione Dati Memorizzati” comprende, oltre al tipo di dato salvato nella memoria, anche le informazioni sulla Data di inizio e fine dell’evento rispettivamente “Tempo1” e “Tempo2” per la registrazione `Reg`, mentre è indicata la Data e Ora per il campionamento istantaneo `Istant`.
Le seguenti operazioni sono possibili sulla videata di Fig. 78:

1.  Usare i tasti freccia alto o basso per evidenziare su sfondo blu una delle registrazioni presenti sulla videata. Premere il tasto `F1` (oppure la voce `INFO` a display). Lo strumento fornisce la videata con le principali informazioni relative alla registrazione effettuata come descritto nel § 5.5.1).
...
Figura 79: Analisi Registrazione (tipo dati Reg)
...
2.  Premere il tasto `ESC` (o la `smart icon`) per uscire dalla funzione e tornare alla pagina “Dati Memorizzati” (Fig. 78).
...
Figura 80: Info Registrazione

1.  In tali condizioni il tasto `F1` (o la voce `MODIFICA` a display) risulta attivo ed è possibile modificare e salvare la riga di commenti usando la tastiera virtuale (vedere il § 5.4.2.1).
2.  Premere il tasto `ESC` (o la `smart icon`) per uscire dalla funzione e tornare alla pagina “Analisi Registrazione” (Fig. 79).

#### 5.5.1.2. Grafico Registrazione

Selezionando l’opzione Grafico Registrazione si accede alla seguente pagina che consente la visualizzazione dell’andamento della registrazione (UNO SOLO parametro per volta).

Figura 81: Selezionare un Parametro

1.  Premere il tasto `F1` (o la voce `PARAM` a display) per accedere alla pagina dei parametri selezionabili (Fig. 82).

Figura 82: Selezione Parametri
...
*   Il tasto `ENTER` (o la `smart icon` a display) confermano la scelta effettuata precedentemente e visualizzano il grafico del parametro scelto (Fig. 83)
*   Il tasto `CANC` deseleziona il parametro precedentemente selezionato, indipendentemente dalla posizione del cursore
*   Il tasto `ESC` (o la `smart icon`) per uscire dalla funzione e tornare alla pagina “Selezionare un Parametro” (Fig. 81)
...
Figura 83: Grafico Registrazione

All’interno di questa pagina sono attivi i seguenti tasti:

*   Il tasto `F1` (o la voce `PARAM` sul display) per accedere alla pagina dei parametri selezionabili (Fig. 82)
*   Il tasto `F4` (o la voce `OPZIONI` a display) per accedere alla pagina di attivazione di “Grafico Avanzato” (Fig. 84)
*   Il tasto `ESC` (o la `smart icon`) per uscire dalla funzione e tornare alla pagina “Analisi Registrazione” (Fig. 79).
2.  Premere il tasto `F4` (o la voce `OPZIONI` a display) per accedere alla pagina di attivazione di Grafico Avanzato (Fig. 84).

Figura 84: Opzioni (Grafico Avanzato)
...
Supponiamo di avere una registrazione di 2000 punti. Il PQA ha un display con risoluzione utile di circa 200 punti, per cui non è in grado di mostrare distintamente tutti i punti della nostra registrazione. Come opera quindi? Il primo punto dei grafici delle curve “Valore Max”, Valore Medio” e Valore Min” saranno l’analisi dei corrispondenti primi 10 punti della registrazione ovvero:

*   Per il grafico del **Valore Max** sarà visualizzato il valore più grande fra i 10 valori registrati per il corrispondente canale
*   Per il grafico del **Valore Min** sarà visualizzato il valore più piccolo fra i 10 valori registrati per il corrispondente canale
*   Per il grafico del **Valore Medio** sarà visualizzato il primo valore fra i 10 registrati per il corrispondente canale.

Analogamente il secondo pixel sarà l’analisi dei successivi 10 punti e così via fino a tracciare tutto il grafico.
3.  Premere il tasto `ESC` (o la `smart icon`) per uscire dalla funzione e tornare alla pagina “Selezionare un Parametro” (Fig. 81).
...
*   Il tasto `F2` (o la voce `PARAM` sul display) accede alla pagina che mostra le impostazioni eseguite per le Anomalie di Tensione (Fig. 86)
*   Il tasto `ESC` (o la `smart icon`) per uscire dalla funzione e tornare alla pagina “Analisi Registrazione” (Fig. 79).

Questa pagina mostra i parametri impostati prima della registrazione per le Anomalie di Tensione:

Figura 86: Parametri Anomalie

Vengono riportati i seguenti parametri non modificabili (essendo stati impostati al momento in cui si è avviata la registrazione):
...
*   Il tasto `ESC` (o la `smart icon`) per uscire dalla funzione e tornare alla pagina “Anomalie” (Fig. 85)

#### 5.5.1.4. Analisi energia consumata

Questa pagina mostra i valori di Energia e Potenza di Picco consumati durante la registrazione.

Figura 87: Videata energia totale consumata
...
> **ATTENZIONE**
> La pagina dell’Energia Totale Consumata è mostrata solo se è stata effettuata la registrazione del Parametro Generale “Potenza &Energia Attiva” Totale (vedere il § 5.4.6.1).

#### 5.5.1.5. Analisi energia prodotta

Questa pagina mostra i valori di Energia e Potenza di Picco prodotti durante la registrazione.

Figura 88: Videata energia totale prodotta
...
> **ATTENZIONE**
> La pagina dell’Energia Totale Prodotta è mostrata solo se è stata effettuata la registrazione del Parametro Generale “Potenza &Energia Attiva” Totale (vedere il § 5.4.6.1). ed abilitato il parametro di Registrazione “Co-Generazione” (vedere il § 5.4.5).
...
Figura 89: Analisi Registrazione (tipo dati Istant)
...
Figura 90: Info Registrazione

1.  In tali condizioni il tasto `F1` (o la voce `MODIFICA` a display) risulta attivo ed è possibile modificare e salvare la riga di commenti usando la tastiera virtuale (vedere il § 5.4.2.1).
2.  Premere il tasto `ESC` (o la `smart icon`) per uscire dalla funzione e tornare alla pagina “Analisi Registrazione” (Fig. 89).

<!-- Chunk: Pages 63-92 -->
### 5.5.2.2. Grafico

In questa videata (Fig. 91) vengono visualizzate simultaneamente i valori istantanei salvati delle forme d’onda delle tensioni V1, V2, V3, Vn scalate rispetto al Fondo Scala (Fs) ed i rispettivi valori efficaci; tali valori sono stati salvati dallo strumento al momento della pressione del tasto `SAVE`. A titolo di esempio è mostrata di seguito la videata di un sistema *Trifase 4-fil*i. Considerazioni analoghe valgono per ogni altro sistema di misura selezionato.

Fig. 91: Scope tensioni in sistema Trifase 4-fili
...
*   Il tasto `ESC` (o la smart icon ` `) per uscire dalla funzione e tornare alla pagina “Analisi Registrazione” (Fig. 89).

### 5.5.2.3. Analisi Armonica

In questa videata (Fig. 92) vengono visualizzati i valori delle armoniche e del THD% della tensione V1, V2, V3, Vn e delle correnti I1, I2, I3 e della Corrente di Neutro In in forma Grafica (scalati rispetto al Fondo Scala Fs) o Tabellare. I valori visualizzati sono in percentuale della fondamentale o in valore assoluto in funzione dell’impostazione del menù `IMPOSTAZIONI ANALIZZATORE -> AVANZATE`. A titolo di esempio è mostrata di seguito la videata di un sistema *Trifase 4-fili*. Considerazioni analoghe valgono per ogni altro sistema di misura selezionato.

Fig. 92: Armoniche totali in sistema Trifase 4-fili
...
*   Il tasto `F4` (o la voce `ARM PAG` sul display) visualizza il successivo gruppo di armoniche: 0..4, 10..16, 22..28, 34..39..40..46..52..58..64
*   Il tasto `ESC` (o la smart icon ` `) per uscire dalla funzione e tornare alla pagina “Analisi Registrazione” (Fig. 89)

### 5.5.2.4. Vettori

A titolo di esempio è mostrata di seguito la videata di un *sistema Trifase 4-fili*. Considerazioni analoghe valgono per ogni altro sistema di misura selezionato. In questa videata (Fig. 93) vengono visualizzati, con indicazioni grafiche e numeriche, gli angoli di sfasamento, espressi in gradi [°] tra:
...
Fig. 93: Diagramma vettoriale totale in sistema Trifase 4-fili
...
A titolo di esempio è mostrata di seguito la videata di un *sistema Trifase 4-fili*. Considerazioni analoghe valgono per ogni altro sistema di misura selezionato. Nella funzione misure lo strumento visualizza i valori salvati in TRMS come mostrato nelle seguenti figure: In questa pagina sono utilizzati i seguenti simboli:
...
*   **SEQ**: Indicazione senso ciclico delle fasi:
    *   ”123” => Corretto
    *   ”132” => Non corretto
    *   ”023” => Tensione nulla su B1
    *   ”103” => Tensione nulla su B2
    *   ”120” => Tensione nulla su B3
    *   ”100” => Tensione nulla su B2 e B3
    *   ”020” => Tensione nulla su B1 e B3
    *   ”003” => Tensione nulla su B1 e B2
*   **Hz**: Frequenza
*   **I1**: Corrente sulla Fase L1
*   **I2**: Corrente sulla Fase L2
*   **I3**: Corrente sulla Fase L3
...
Fig. 94: Pagina 1/5 dei valori numerici per Sistema Trifase 4 fili

All’interno di questa pagina sono attivi i seguenti tasti:
*   Il tasto `F1` (o la voce `PAG` sul display) avanza alle pagine successive di valori salvati
*   Il tasto `ESC` (o la smart icon ` `) per uscire dalla funzione e tornare alla pagina “Analisi Registrazione” (Fig. 89).

## 5.5.3. Trasferimento di registrazioni su Pen Driver USB esterno

È possibile effettuare il trasferimento di una o più registrazioni (una per volta) presenti sulla videata di Fig. 78 ad un Pen Driver USB esterno collegato allo strumento (vedere la Fig. 3). Lo strumento presenta la videata seguente:

Fig. 95: Videata Gestione Dati Memorizzati con inserimento Pen Driver USB
...
Fig. 96: Definizione nome del file trasferito su Pen Driver USB
...
Fig. 97: Conferma sovrascrittura del file
...
## 5.5.4. Salvataggio registrazioni su Compact Flash esterna
...
Fig. 98: Esempio di uso della memoria esterna - non presenza Compact Flash

Nell’esempio di Fig. 98 si è considerata una configurazione predefinita “POTENZA & ENERGIA che comporta una autonomia di 540 g 2 h in assenza di Compact Flash esterna. Introducendo la Compact Flash, uscendo dalla videata premendo il tasto `ESC` (o la smart icon ` `) e rientrando nella stessa sezione, lo strumento presenta la videata seguente:

Fig. 99: Esempio di uso della memoria esterna - presenza Compact Flash

L’inserimento della Compact Flash comporta la presenza dell’icona sulla parte alta del display e un aumento dell’autonomia in tempo reale (1941 g 19 h nell’esempio di Fig. 99).
...
2.  Verificare la presenza dell’icona `ActiveSync` con sfondo **grigio** nella parte bassa destra del PC, in corrispondenza della barra dei processi in esecuzione, come evidenziato nella Fig. 102:

    Fig. 102: Icona ActiveSync non attiva
3.  Collegare lo strumento al PC usando il cavo C2007 (USB “A” → USB “B”) fornito in dotazione come mostrato in Fig. 103:

    Fig. 103: Collegamento strumento a PC
4.  Verificare che l’icona `ActiveSync` presente nella parte bassa destra del PC in corrispondenza della barra dei processi in esecuzione sia con sfondo **verde**, come mostrato in Fig. 104. In tal modo lo strumento comunica regolarmente con il PC:

    Fig. 104: Icona ActiveSync attiva
5.  Lanciare il software `TopView` e cliccare sul tasto “`Coll. PC <-> Strum.`” Premere il tasto “`Rileva strumento`“ nella videata del software al fine del riconoscimento dello strumento collegato, visibile nella barra di stato corrispondente. Selezionare il modello “`VEGA78`” nell’elenco degli strumenti disponibili solo durante la prima connessione.
...
> ATTENZIONE
> La massima tensione fra gli ingressi B1, B2, B3, B4 e BE è 1000V / CAT IV 600V verso Terra. Non misurare tensioni che eccedono i limiti indicati in questo manuale. Il superamento dei limiti di tensione potrebbe causare shock elettrici all’utilizzatore e danni allo strumento.

Fig. 105: Collegamento dello strumento in un sistema Monofase
...
2.  Collegare i cavi di misura della tensione sui conduttori di Fase, Neutro e Terra rispettando i colori come mostrato in Fig. 105.
3.  Collegare la pinza di corrente sul conduttore di Fase L1 come mostrato nella Fig. 105 rispettando il verso della freccia presente sulla pinza stessa che indica la direzione convenzionale della corrente da generatore a carico. Verificare sempre, nelle videate della sezione Visualizzazione Misure, che la Potenza Attiva P sia sempre positiva e che il fattore di potenza Pf sia corrispondente al carico, normalmente induttivo, prima di attivare una registrazione. In caso di valori negativi della potenza attiva ruotare la pinza di 180° sul conduttore.
4.  Alimentare il sistema elettrico in esame qualora questo fosse stato messo momentaneamente fuori servizio per il collegamento dello strumento. I valori delle grandezze elettriche disponibili sono mostrati a display nella sezione Visualizzazione Misure (vedere il par 5.2).
5.  Se si intende salvare un campionamento dei valori presenti a display (Istant) premere il tasto `SAVE` (vedere il § 5.5). Usare eventualmente la funzione `HOLD` per congelare a display i valori delle grandezze visualizzate.
6.  Controllare le impostazioni prima di attivare una registrazione (vedere il par. 5.4). Premere il tasto `GO/STOP` per attivare/disattivare una registrazione sullo strumento (rif. par 5.4.10).
...
Fig. 106: Collegamento dello strumento in un sistema Trifase 4 - fili
...
2.  Collegare i cavi di misura della tensione sui conduttori di Fase L1, L2, L3, Neutro e Terra rispettando i colori come mostrato in Fig. 106. Verificare a display l’indicazione “123” relativa al corretto senso ciclico delle fasi (vedere il § 5.2.1) ed effettuare le opportune correzioni fino al raggiungimento di tale situazione.
3.  Collegare le pinze di corrente sui conduttori di Fase L1, L2, L3 e Neutro N come mostrato nella Fig. 106 rispettando il verso della freccia presente sulle pinze stesse che indica la direzione convenzionale della corrente da generatore a carico. Verificare sempre, nelle videate della sezione Visualizzazione Misure, che le Potenze Attive P1, P2, P3 siano sempre positive e che i fattori di potenza Pf1, Pf2, Pf3 siano corrispondenti ad un carico, normalmente induttivo, prima di attivare una registrazione. In caso di valori negativi delle potenze attiva ruotare le pinze di 180° sui conduttori interessati.
4.  Alimentare il sistema elettrico in esame qualora questo fosse stato messo momentaneamente fuori servizio per il collegamento dello strumento. I valori delle grandezze elettriche disponibili sono mostrati a display nella sezione Visualizzazione Misure (vedere il § 5.2).
5.  Se si intende salvare un campionamento dei valori presenti a display (Istant) premere il tasto `SAVE` (vedere il § 5.5). Usare eventualmente la funzione `HOLD` per congelare a display i valori delle grandezze visualizzate.
6.  Controllare le impostazioni prima di attivare una registrazione (vedere il § 5.4). Premere il tasto `GO/STOP` per attivare e disattivare una registrazione sullo strumento (vedere il § 5.4.10).
...
Fig. 107: Collegamento dello strumento in un sistema Trifase 3 - fili
...
2.  Collegare i cavi di misura della tensione sui conduttori di Fase L1, L2, L3 e Terra rispettando i colori come mostrato in Fig. 107. Verificare a display l’indicazione “123” relativa al corretto senso ciclico delle fasi (vedere il § 5.2.1) ed effettuare le opportune correzioni fino al raggiungimento di tale situazione.
3.  Collegare le pinze di corrente sui conduttori di Fase L1, L2, L3 come mostrato nella Fig. 107 rispettando il verso della freccia presente sulla pinza stessa che indica la direzione convenzionale della corrente da generatore a carico. Verificare sempre, nelle videate della sezione Visualizzazione Misure, che le Potenze Attive P1, P2, P3 siano sempre positive e che i fattori di potenza Pf1, Pf2, Pf3 siano corrispondenti ad un carico, normalmente induttivo, prima di attivare una registrazione. In caso di valori negativi delle potenze attive ruotare le pinza di 180° sui conduttori interessati.
4.  Alimentare il sistema elettrico in esame qualora questo fosse stato messo momentaneamente fuori servizio per il collegamento dello strumento. I valori delle grandezze elettriche disponibili sono mostrati a display nella sezione Visualizzazione Misure (vedere il § 5.2).
5.  Se si intende salvare un campionamento dei valori presenti a display (Istant) premere il tasto `SAVE` (vedere il § 5.5). Usare eventualmente la funzione `HOLD` per congelare a display i valori delle grandezze visualizzate.
6.  Controllare le impostazioni prima di attivare una registrazione (vedere il § 5.4). Premere il tasto `GO/STOP` per attivare e disattivare una registrazione sullo strumento (vedere il § 5.4.10).
...
Fig. 108: Collegamento dello strumento in un sistema Trifase 3 - fili ARON
...
2.  Collegare i cavi di misura della tensione sui conduttori di Fase L1, L2, L3 e eventualmente Terra rispettando i colori come mostrato in Fig. 108. Verificare a display l’indicazione “123” relativa al corretto senso ciclico delle fasi (vedere il § 5.2.1) ed effettuare le opportune correzioni fino al raggiungimento di tale situazione.
3.  Collegare le pinze di corrente sui conduttori di Fase L1, L2, L3 come mostrato nella Fig. 108 rispettando il verso della freccia presente sulla pinze stesse che indica la direzione convenzionale della corrente da generatore a carico. Verificare sempre, (selezionando preliminarmente il sistema 3-fili) nelle videate della sezione Visualizzazione Misure, che le Potenze Attive P1, P2, P3 siano sempre positive e che i fattori di potenza Pf1, Pf2, Pf3 siano corrispondenti ad un carico, normalmente induttivo, prima di attivare una registrazione. In caso di valori negativi della potenze attive ruotare le pinza di 180° sui conduttori interessati.
4.  Alimentare il sistema elettrico in esame qualora questo fosse stato messo momentaneamente fuori servizio per il collegamento dello strumento. I valori delle grandezze elettriche disponibili sono mostrati a display nella sezione Real Time (vedere il § 5.2).
5.  Se si intende salvare un campionamento dei valori presenti a display (Istant) premere il tasto `SAVE` (vedere il § 5.5). Usare eventualmente la funzione `HOLD` per congelare a display i valori delle grandezze visualizzate.
6.  Controllare le impostazioni prima di attivare una registrazione (vedere il § 5.4). Premere il tasto `GO/STOP` per attivare e disattivare una registrazione sullo strumento (vedere il § 5.4.10).
...
Fig. 109: Collegamento dello strumento in un sistema Trifase 4 - fili 3F HL
...
2.  Collegare i cavi di misura della tensione sui conduttori di Fase L1, L2, L3, N e PE rispettando i colori come mostrato in Fig. 109. Verificare a display l’indicazione “123” relativa al corretto senso ciclico delle fasi (vedere il § 5.2.1) ed effettuare le opportune correzioni fino al raggiungimento di tale situazione.
3.  Collegare le pinze di corrente sui conduttori di Fase L1, L2, L3 e N come mostrato nella Fig. 109 rispettando il verso della freccia presente sulla pinze stesse che indica la direzione convenzionale della corrente da generatore a carico. Verificare sempre, (selezionando preliminarmente il sistema 3-fili) nelle videate della sezione Visualizzazione Misure, che le Potenze Attive P1, P2, P3 siano sempre positive e che i fattori di potenza Pf1, Pf2, Pf3 siano corrispondenti ad un carico, normalmente induttivo, prima di attivare una registrazione. In caso di valori negativi della potenze attive ruotare le pinza di 180° sui conduttori interessati.
4.  Alimentare il sistema elettrico in esame qualora questo fosse stato messo momentaneamente fuori servizio per il collegamento dello strumento. I valori delle grandezze elettriche disponibili sono mostrati a display nella sezione Real Time (vedere il § 5.2).
5.  Se si intende salvare un campionamento dei valori presenti a display (Istant) premere il tasto `SAVE` (vedere il § 5.5). Usare eventualmente la funzione `HOLD` per congelare a display i valori delle grandezze visualizzate.
6.  Controllare le impostazioni prima di attivare una registrazione (vedere il § 5.4). Premere il tasto `GO/STOP` per attivare e disattivare una registrazione sullo strumento (vedere il § 5.4.10).

### 7.6. USO DELLO STRUMENTO IN UN SISTEMA 3 - FILI 3 F DA (SISTEMI USA)

> ATTENZIONE
> La massima tensione fra gli ingressi B1, B2, B3, B4 e BE è 1000V / CAT IV 600V verso Terra. Non misurare tensioni che eccedono i limiti indicati in questo manuale. Il superamento dei limiti di tensione potrebbe causare shock elettrici all’utilizzatore e danni allo strumento.

Fig. 110: Collegamento dello strumento in un sistema Trifase 3 - fili 3F DA
...
2.  Collegare i cavi di misura della tensione sui conduttori di Fase L1, L2, L3 e PE rispettando i colori come mostrato in Fig. 110. Verificare a display l’indicazione “123” relativa al corretto senso ciclico delle fasi (vedere il § 5.2.1) ed effettuare le opportune correzioni fino al raggiungimento di tale situazione.
3.  Collegare le pinze di corrente sui conduttori di Fase L1, L2, L3 come mostrato nella Fig. 110 rispettando il verso della freccia presente sulla pinze stesse che indica la direzione convenzionale della corrente da generatore a carico. Verificare sempre, (selezionando preliminarmente il sistema 3-fili) nelle videate della sezione Visualizzazione Misure, che le Potenze Attive P1, P2, P3 siano sempre positive e che i fattori di potenza Pf1, Pf2, Pf3 siano corrispondenti ad un carico, normalmente induttivo, prima di attivare una registrazione. In caso di valori negativi della potenze attive ruotare le pinza di 180° sui conduttori interessati.
4.  Alimentare il sistema elettrico in esame qualora questo fosse stato messo momentaneamente fuori servizio per il collegamento dello strumento. I valori delle grandezze elettriche disponibili sono mostrati a display nella sezione Real Time (vedere il § 5.2).
5.  Se si intende salvare un campionamento dei valori presenti a display (Istant) premere il tasto `SAVE` (vedere il § 5.5). Usare eventualmente la funzione `HOLD` per congelare a display i valori delle grandezze visualizzate.
6.  Controllare le impostazioni prima di attivare una registrazione (vedere il § 5.4). Premere il tasto `GO/STOP` per attivare e disattivare una registrazione sullo strumento (vedere il § 5.4.10).
...
Fig. 111: Collegamento dello strumento in un sistema Bifase 3 - fili 3F 2E
...
2.  Collegare i cavi di misura della tensione sui conduttori di Fase L1, L2, N e PE rispettando i colori come mostrato in Fig. 111.
3.  Collegare le pinze di corrente sui conduttori di Fase L1, L2 e Neutro N come mostrato nella Fig. 111 rispettando il verso della freccia presente sulla pinze stesse che indica la direzione convenzionale della corrente da generatore a carico. Verificare sempre, (selezionando preliminarmente il sistema 3-fili) nelle videate della sezione Visualizzazione Misure, che le Potenze Attive P1, P2 siano sempre positive e che i fattori di potenza Pf1, Pf2 siano corrispondenti ad un carico, normalmente induttivo, prima di attivare una registrazione. In caso di valori negativi della potenze attive ruotare le pinza di 180° sui conduttori interessati.
4.  Alimentare il sistema elettrico in esame qualora questo fosse stato messo momentaneamente fuori servizio per il collegamento dello strumento. I valori delle grandezze elettriche disponibili sono mostrati a display nella sezione Real Time (vedere il § 5.2).
5.  Se si intende salvare un campionamento dei valori presenti a display (Istant) premere il tasto `SAVE` (vedere il § 5.5). Usare eventualmente la funzione `HOLD` per congelare a display i valori delle grandezze visualizzate.
6.  Controllare le impostazioni prima di attivare una registrazione (vedere il § 5.4). Premere il tasto `GO/STOP` per attivare e disattivare una registrazione sullo strumento (vedere il § 5.4.10).
...
Fig. 112: Collegamento dello strumento in un sistema Bifase 3 - fili 3F YA
...
2.  Collegare i cavi di misura della tensione sui conduttori di Fase L1, L2, N e PE rispettando i colori come mostrato in Fig. 112.
3.  Collegare le pinze di corrente sui conduttori di Fase L1, L2 e Neutro N come mostrato nella Fig. 112 rispettando il verso della freccia presente sulla pinze stesse che indica la direzione convenzionale della corrente da generatore a carico. Verificare sempre, (selezionando preliminarmente il sistema 3-fili) nelle videate della sezione Visualizzazione Misure, che le Potenze Attive P1, P2 siano sempre positive e che i fattori di potenza Pf1, Pf2 siano corrispondenti ad un carico, normalmente induttivo, prima di attivare una registrazione. In caso di valori negativi della potenze attive ruotare le pinza di 180° sui conduttori interessati.
4.  Alimentare il sistema elettrico in esame qualora questo fosse stato messo momentaneamente fuori servizio per il collegamento dello strumento. I valori delle grandezze elettriche disponibili sono mostrati a display nella sezione Real Time (vedere il § 5.2).
5.  Se si intende salvare un campionamento dei valori presenti a display (Istant) premere il tasto `SAVE` (vedere il § 5.5). Usare eventualmente la funzione `HOLD` per congelare a display i valori delle grandezze visualizzate.
6.  Controllare le impostazioni prima di attivare una registrazione (vedere il § 5.4). Premere il tasto `GO/STOP` per attivare e disattivare una registrazione sullo strumento (vedere il § 5.4.10).
...
Fig. 113: Collegamento dello strumento in un sistema Bifase 3 - fili 1 F PC

> ATTENZIONE
> Ove possibile togliere alimentazione al sistema elettrico in esame prima di effettuare il collegamento dello strumento.

1.  Controllare ed eventualmente modificare le impostazioni di base dello strumento (vedere il § 5.3.1). Impostare la configurazione `1 F PC`.
2.  Collegare i cavi di misura della tensione sui conduttori di Fase L1, L2, N e PE rispettando i colori come mostrato in Fig. 113.
3.  Collegare le pinze di corrente sui conduttori di Fase L1, L2 e Neutro N come mostrato nella Fig. 113 rispettando il verso della freccia presente sulla pinze stesse che indica la direzione convenzionale della corrente da generatore a carico. Verificare sempre, (selezionando preliminarmente il sistema 3-fili) nelle videate della sezione Visualizzazione Misure, che le Potenze Attive P1, P2 siano sempre positive e che i fattori di potenza Pf1, Pf2 siano corrispondenti ad un carico, normalmente induttivo, prima di attivare una registrazione. In caso di valori negativi della potenze attive ruotare le pinza di 180° sui conduttori interessati.
4.  Alimentare il sistema elettrico in esame qualora questo fosse stato messo momentaneamente fuori servizio per il collegamento dello strumento. I valori delle grandezze elettriche disponibili sono mostrati a display nella sezione Real Time (vedere il § 5.2).
5.  Se si intende salvare un campionamento dei valori presenti a display (Istant) premere il tasto `SAVE` (vedere il § 5.5). Usare eventualmente la funzione `HOLD` per congelare a display i valori delle grandezze visualizzate.
6.  Controllare le impostazioni prima di attivare una registrazione (vedere il § 5.4). Premere il tasto `GO/STOP` per attivare e disattivare una registrazione sullo strumento (vedere il § 5.4.10).
...
Il `VEGA78` è uno strumento di precisione. Durante l’utilizzo e la conservazione rispettare le raccomandazioni elencate in questo manuale per evitare danni o possibili pericoli durante l’uso. Non utilizzare lo strumento in ambienti caratterizzati da elevato tasso di umidità o temperatura elevata. Non esporre direttamente alla luce del sole. Spegnere sempre lo strumento dopo l’utilizzo.
...
Tabella 8: Elenco icone batteria presenti a display

| Icona a display | Descrizione                                     |
| :-------------- | :---------------------------------------------- |
| ![Low Battery Icon](https://via.placeholder.com/20x20?text=LBat) | Livello di batteria troppo basso. Eseguire la ricarica |
| ![25% Battery Icon](https://via.placeholder.com/20x20?text=25%) | Livello di batteria residua pari al 25%             |
| ![50% Battery Icon](https://via.placeholder.com/20x20?text=50%) | Livello di batteria residua pari al 50%             |
| ![75% Battery Icon](https://via.placeholder.com/20x20?text=75%) | Livello di batteria residua pari al 75%             |
| ![Full Battery Icon](https://via.placeholder.com/20x20?text=Full) | Batteria completamente carica                        |
| ![AC Adapter Icon](https://via.placeholder.com/20x20?text=AC) | Presenza del solo alimentatore. Batteria scollegata |
| ![Charging Battery Icon](https://via.placeholder.com/20x20?text=Chg) | Presenza di batteria e alimentatore. Batteria in ricarica |
| ![Full Battery with AC Icon](https://via.placeholder.com/20x20?text=FullAC) | Batteria completamente carica con alimentatore collegato |
| ![Unknown Battery Icon](https://via.placeholder.com/20x20?text=?) | Carica batteria non nota. Collegare l’alimentatore  |
| ![Battery Error Icon](https://via.placeholder.com/20x20?text=Err) | Problemi con la batteria. Contattare il servizio di assistenza HT ITALIA |

*(Note: The actual icon images are not provided in text, so I used placeholder descriptions)*
...
**Tensione TRMS AC/DC Fase - Neutro/Fase - Terra – Sistemi Monofase / Trifase**

| Campo               | Incertezza              | Risoluzione | Impedenza d’ingresso |
| :------------------ | :---------------------- | :---------- | :------------------- |
| 0.0 ÷ 600.0V        | ± (0.5%lettura+2cifre)  | 0.1V        | 10M Ω                |

Fattore di Cresta max = 2
I valori di Tensione < 2.0V vengono azzerati
Lo strumento è collegabile a TV con Fattore impostabile da 1 ÷ 3000

**Tensione TRMS AC/DC Fase - Fase – Sistemi Trifase**

| Campo                | Incertezza              | Risoluzione | Impedenza d’ingresso |
| :------------------- | :---------------------- | :---------- | :------------------- |
| 0.0 ÷ 1000.0V        | ± (0.5%lettura+2cifre)  | 0.1V        | 10M Ω                |

Fattore di Cresta max = 2
I valori di Tensione < 2.0V vengono azzerati
Lo strumento è collegabile a TV con Fattore impostabile da 1 ÷ 3000

**Anomalie di Tensione – Fase - Neutro – Sistemi Monofase / Trifase con Neutro**
...
Fattore di Cresta max = 2
I valori di Tensione < 2.0 V vengono azzerati
Lo strumento è collegabile a TV con Fattore impostabile da 1 ÷ 3000
Soglia impostabile da ± 1% a ± 30%

**Anomalie di Tensione – Fase - Fase – Sistemi Trifase senza Neutro**
...
**Corrente tramite trasduttore Standard STD**

| Campo                 | Incertezza                 | Risoluzione | Impedenza d’ingresso | Protezione Sovraccarichi |
| :-------------------- | :------------------------- | :---------- | :------------------- | :----------------------- |
| 0.0 ÷ 1000.0mV        | ± (0.5%lettura+0.06%FS)    | 0.1mV       | 510k Ω               | 5V                       |

Misura effettuata tramite Pinza con Uscita = 1VAC quando la pinza è soggetta alla corrente nominale.
Fattore di Cresta max = 3
I valori di corrente < 0.1% del FS vengono azzerati.

**Corrente tramite trasduttore FLEX – Portata 300A**

| Campo                | Incertezza                 | Risoluzione | Impedenza d’ingresso | Protezione Sovraccarichi |
| :------------------- | :------------------------- | :---------- | :------------------- | :----------------------- |
| 0.0 ÷ 49.9A          | ± (0.5%lettura+0.24%FS)    | 0.1A        | 510k Ω               | 5V                       |
| 50.0 ÷ 300.0A        | ± (0.5%lettura+0.06%FS)    |             |                      |                          |

Misura effettuata tramite Pinza HTFlex33
Fattore di Cresta max = 3
I valori di corrente < 1A vengono azzerati.

**Corrente tramite trasduttore FLEX – Portata 3000A**

| Campo                 | Incertezza                 | Risoluzione | Impedenza d’ingresso | Protezione Sovraccarichi |
| :-------------------- | :------------------------- | :---------- | :------------------- | :----------------------- |
| 0.0 ÷ 3000.0A         | ± (0.5%lettura+0.06%FS)    | 0.1A        | 510k Ω               | 5V                       |

Misura effettuata tramite Pinza HTFlex33
Fattore di Cresta max = 3
I valori di corrente < 5A vengono azzerati.

**Potenza – Sistemi Monofase e Trifase (@ Cos φ >0.5 e Vmis >60V)**

| Grandezza [W, VAr, VA] | Fondo Scala Pinze FS | Portate [W, VAr, VA] | Incertezza              | Risoluzione [W, VAr, VA] |
| :--------------------- | :------------------- | :------------------- | :---------------------- | :----------------------- |
| Potenza Attiva         | FS ≤ 1A              | 0.0 – 999.9          |                         | 0.1                      |
| Potenza Reattiva       |                      | 1.000 – 9.999k       | ± (1.0%lett+6cifre)     | 0.001k                   |
| Potenza Apparente      | 1A< FS ≤ 10A         | 0.000 – 9.999k       |                         | 0.001k                   |
|                        |                      | 10.00 – 99.99k       |                         | 0.01k                    |
|                        | 10A< FS ≤ 100A       | 0.00 – 99.99k        |                         | 0.01k                    |
|                        |                      | 100.0 – 999.9k       |                         | 0.1k                     |
|                        | 100A< FS ≤ 3000A     | 0.0 – 999.9k         |                         | 0.1k                     |
|                        |                      | 1.000 – 9.999M       |                         | 0.001M                   |

Per Pinze STD
Vmis = Tensione a cui è misurata la Potenza
FS = Fondo Scala Corrente

**Energia – Sistemi Monofase e Trifase (@ Cos φ >0.5 e Vmis >60V)**

| Grandezza [Wh, VArh, VAh] | Fondo Scala Pinze (FS) | Portate [Wh, VArh, VAh] | Incertezza              | Risoluzione [Wh, VArh, VAh] |
| :------------------------ | :--------------------- | :---------------------- | :---------------------- | :-------------------------- |
| Energia Attiva            | FS ≤ 1A                | 0.0 – 999.9             |                         | 0.1                         |
| Energia Reattiva          |                        | 1.000 – 9.999k          | ± (1.0%lett+6cifre)     | 0.001k                      |
| Energia Apparente         | 1A< FS ≤ 10A           | 0.000 – 9.999k          |                         | 0.001k                      |
|                           |                        | 10.00 – 99.99k          |                         | 0.01k                       |
|                           | 10A< FS ≤ 100A         | 0.00 – 99.99k           |                         | 0.01k                       |
|                           |                        | 100.0 – 999.9k          |                         | 0.1k                        |
|                           | 100A< FS ≤ 3000A       | 0.0 – 999.9k            |                         | 0.1k                        |
...
| Campo       | Incertezza (*)               | Risoluzione     |
| :---------- | :--------------------------- | :-------------- |
| DC ÷ 64a    | ± (5.0%lettura+5cifre)       | 0.1V / 0.1A     |
...
*   **Parametri generali della rete**: Tensioni, Correnti, Potenze, Cos φ, Energie Dissimetrie, THD%, Armoniche.
...
**Display**
*   **Caratteristiche**: Grafico colori `TFT` retroilluminato, ¼ “ VGA (320x240)
*   **Touch screen**: Presente
*   **Numero colori**: 65536
...
**Alimentazione**
*   **Alimentazione interna**: Batteria ricaricabile, autonomia > 6 ore
*   **Alimentazione esterna**: Alimentatore 230VAC / 5VDC, 7.5W
*   **Autospegnimento**: Dopo 5 minuti di non utilizzo (in assenza di alimentatore)

**Caratteristiche meccaniche**
*   **Dimensioni (L x La x H)**: 235 x 165 x 75 mm
*   **Peso (batteria inclusa)**: Circa 1.0kg
...
*   **Categoria di sovratensione**: `CAT IV 600V` verso Terra, max 1000V tra gli ingressi
*   **Qualità di rete elettrica**: `EN50160`
...
Lo strumento definisce come anomalie di tensione tutti i valori TRMS, calcolati ogni 10ms, esternamente alle soglie impostate in fase di programmazione da ± 1% a ± 30% rispetto ad un valore fissato come riferimento con step dell’1%.
...
Si può osservare come nella (1) l’indice della sommatoria vada da 1 a infinito. Quello che accade in realtà è che ogni segnale non ha un numero di armoniche illimitato: esiste sempre un numero d’ordine oltre il quale il valore delle armoniche è trascurabile.
...
*(Fig. 114: Effetto della somma di 2 frequenze multiple - Image not provided, but described)*

La normativa EN50160 suggerisce di troncare la sommatoria nell’espressione (1) alla 40a armonica. Un indice fondamentale per rilevare la presenza di armoniche è il parametro Distorsione Armonica Totale THD% (valore percentuale) definito come:

```
THDV% = (√(Σ(h=2 to 40) Vh²)) / V1 * 100
```
...
Tabella 9: Valori di riferimento armoniche di tensione secondo la EN50160
...
Qualsiasi utilizzatore che alteri l'onda sinusoidale o usi soltanto una parte di detta onda causa distorsioni alla sinusoide e quindi armoniche.
...
### 10.3. DISSIMMETRIA DELLE TENSIONI DI ALIMENTAZIONE
...
Fig. 114: Scomposizione di una terna di vettori *(Image not provided)*
...
Lo strumento `VEGA78` consente la misura e registrazione dei seguenti parametri che definiscono la percentuale della dissimmetria sulle tensioni di un sistema elettrico:

```
INV% = (Ei / Ed) * 100% = componente a sequenza inversa
OMO% = (E0 / Ed) * 100% = componente a sequenza omopolare
```
...
*   **Fattore di Potenza di fase**: (n=1,2,3) PF_n = Pn / Sn
*   **Potenza Attiva Totale**: P_TOT = P1 + P2 + P3
*   **Potenza Reattiva totale**: Q_TOT = Q1 + Q2 + Q3
*   **Potenza Apparente Totale**: S_TOT = √(P_TOT² + Q_TOT²)
*   **Fattore di Potenza Totale**: PF_TOT = P_TOT / S_TOT
...
*   **Potenza Attiva di fase**: (n=1,2,3) Pn = Σ(k=0 to ∞) Vk,n ⋅ Ik,n ⋅ cos(φk,n)
*   **Potenza Apparente di fase**: (n=1,2,3) Sn = VnN ⋅ In
*   **Potenza Reattiva di fase**: (n=1,2,3) Qn = √(Sn² - Pn²)
*   **Fattore di Potenza di fase**: (n=1,2,3) PF_n = Pn / Sn
*   **Fattore di Potenza distorto**: (n=1,2,3) dPFn = cos φ1n = sfasamento fra le fondamentali di tensione e corrente della fase n
*   **Potenza Attiva Totale**: P_TOT = P1 + P2 + P3
*   **Potenza Reattiva Totale**: Q_TOT = Q1 + Q2 + Q3
*   **Potenza Apparente Totale**: S_TOT = √(P_TOT² + Q_TOT²)
*   **Fattore di Potenza Totale**: PF_TOT = P_TOT / S_TOT
...
dove:
*   **Vk,n**: Valore efficace della k-esima armonica di tensione fra la fase n ed il Neutro.
*   **Ik,n**: Valore efficace della k-esima armonica di corrente della fase n.
*   **φk,n**: Angolo di sfasamento tra la k-esima armonica di tensione e la k-esima armonica di corrente della fase n.
...
#### 10.4.1. Convenzioni sulle Potenze e Fattori di Potenza
...
*(Representing the quadrant diagram in plain text is challenging. I will describe the convention tables below.)*
...
*(Angle ranges not explicitly given, but implied by quadrants)*
P + = 0, Pfc + = -1, Pfi + = -1, Qc + = 0, Qi + = 0
P - = P, Pfc - = Pf, Pfi - = -1, Qc - = Q, Qi - = 0
P + = P, Pfc + = -1, Pfi + = Pf, Qc + = 0, Qi + = Q
P - = 0, Pfc - = -1, Pfi - = -1, Qc - = 0, Qi - = 0
...
*(The diagram shows a Trifase Utenza with Phase 1, Phase 2, Phase 3 and connections to W1-2, VAR1-2, W3-2, VAR3-2)*
...
P_TOT = W1-2 + W3-2
Q_TOT = VAR1-2 + VAR3-2
S_TOT = √((W1-2 + W3-2)² + (VAR1-2 + VAR3-2)²)
```

### 10.5. CENNI SUL METODO DI MISURA
...
#### 10.5.1. Uso del periodo di integrazione
...
#### 10.5.2. Calcolo del fattore di potenza
